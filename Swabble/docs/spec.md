# swabble â€?macOS 26 speech hook daemon (Swift 6.2)
...
- Simple config at `~/.config/swabble/config.json` (JSON, Codable) â€?no TOML.
...
- **Audio + Speech pipeline**: `SpeechPipeline` wraps `AVAudioEngine` input â†?`SpeechAnalyzer` with `SpeechTranscriber` module. Emits partial/final transcripts via async stream. Requests `.audioTimeRange` when transcripts enabled. Handles Speech permission and asset download prompts ahead of capture.
