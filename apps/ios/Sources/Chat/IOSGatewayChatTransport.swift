...
struct IOSGatewayChatTransport: openxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> openxChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(openxChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> openxChatHistoryPayload {
        ...
        return try JSONDecoder().decode(openxChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [openxChatAttachmentPayload]) async throws -> openxChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [openxChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(openxChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(openxGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openxGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openxChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: openxAgentEventPayload.self)
                        {
