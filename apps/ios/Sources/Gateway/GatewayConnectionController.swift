...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [openxCapability.canvas.rawValue, openxCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(openxCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(openxCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = openxLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(openxCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            openxCanvasCommand.present.rawValue,
            openxCanvasCommand.hide.rawValue,
            openxCanvasCommand.navigate.rawValue,
            openxCanvasCommand.evalJS.rawValue,
            openxCanvasCommand.snapshot.rawValue,
            openxCanvasA2UICommand.push.rawValue,
            openxCanvasA2UICommand.pushJSONL.rawValue,
            openxCanvasA2UICommand.reset.rawValue,
            openxScreenCommand.record.rawValue,
            openxSystemCommand.notify.rawValue,
            openxSystemCommand.which.rawValue,
            openxSystemCommand.run.rawValue,
            openxSystemCommand.execApprovalsGet.rawValue,
            openxSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(openxCapability.camera.rawValue) {
            commands.append(openxCameraCommand.list.rawValue)
            commands.append(openxCameraCommand.snap.rawValue)
            commands.append(openxCameraCommand.clip.rawValue)
        }
        if caps.contains(openxCapability.location.rawValue) {
            commands.append(openxLocationCommand.get.rawValue)
        }
