...
struct SettingsTab: View {
    ...
    @AppStorage("location.enabledMode") private var locationEnabledModeRaw: String = openxLocationMode.off.rawValue
    ...
    @State private var lastLocationModeRaw: String = openxLocationMode.off.rawValue
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section("Location") {
                    Picker("Location Access", selection: self.$locationEnabledModeRaw) {
                        Text("Off").tag(openxLocationMode.off.rawValue)
                        Text("While Using").tag(openxLocationMode.whileUsing.rawValue)
                        Text("Always").tag(openxLocationMode.always.rawValue)
                    }
                ...
                Section("Screen") {
                    ...
                    Text("Keeps the screen awake while openx is open.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            .onChange(of: self.locationEnabledModeRaw) { _, newValue in
                ...
                guard let mode = openxLocationMode(rawValue: newValue) else { return }
                Task {
    ...
    private var locationMode: openxLocationMode {
        openxLocationMode(rawValue: self.locationEnabledModeRaw) ?? .off
    }
    ...
    private func gatewayDetailLines(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) -> [String] {
        ...
        if gatewayPort != nil || canvasPort != nil {
            let gw = gatewayPort.map(String.init) ?? "â€?
            let canvas = canvasPort.map(String.init) ?? "â€?
            lines.append("Ports: gateway \(gw) Â· canvas \(canvas)")
        }
