...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: openxOAuthStore.AnthropicOAuthStatus = openxOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: openxOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = openxOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = openxOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try openxOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. openx can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: openxOAuthStore.AnthropicOAuthStatus,
