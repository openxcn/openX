...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (openx token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: openxOAuthStore.AnthropicOAuthStatus = openxOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "bot.openx", category: "anthropic-oauth")
...
enum openxOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let openxOAuthDirEnv = "CLAWDBOT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "openx OAuth token file not found"
            case .unreadableFile: "openx OAuth token file not readable"
            case .invalidJSON: "openx OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in openx OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "openx OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.openxOAuthDirEnv]?
