...
enum CommandResolver {
    private static let projectRootDefaultsKey = "openx.gatewayProjectRootPath"
    private static let helperName = "openx"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/openx.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/openx")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let openxPaths = self.openxManagedPaths(home: home)
        if !openxPaths.isEmpty {
            extras.insert(contentsOf: openxPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + openxPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func openxManagedPaths(home: URL) -> [String] {
    ...
    static func openxExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectopenxExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/openx.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyopenxInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.openxExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let openxPath = self.projectopenxExecutable(projectRoot: root) {
                return [openxPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "openx", subcommand] + extraArgs
            }
            if let openxPath = self.openxExecutable(searchPaths: searchPaths) {
                return [openxPath, subcommand] + extraArgs
            }

            let missingEntry = """
            openx entrypoint missing (looked for dist/index.js or bin/openx.js); run pnpm build.
            """
    ...
    // Existing callers still refer to openxCommand; keep it as node alias.
    static func openxCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real openx CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/openx"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/openx.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/openx.js"
            node "$PRJ/bin/openx.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent openx"
          pnpm --silent openx \(quotedArgs);
        else
          echo "openx CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? openxConfigFile.loadDict()
