...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: openxProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "bot.openx", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: openxKit.AnyCodable]()) {
                $0[$1.key] = openxKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: openxProtocol.AnyCodable?) -> [String: openxProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: openxProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: openxKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: openxProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("openx.control.heartbeat")
    static let controlAgentEvent = Notification.Name("openx.control.agent")
}
