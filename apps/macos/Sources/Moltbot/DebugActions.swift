...
enum DebugActions {
    private static let verboseDefaultsKey = "openx.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "openx.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "openx", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".clawdbot/openx.json")
        guard
