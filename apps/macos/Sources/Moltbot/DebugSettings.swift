    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "When enabled, openx won't install or manage \(gatewayLaunchdLabel). " +
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "openx://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (openx://â€? are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    ScrollView {
                        Text(self.gatewayManager.log.isEmpty ? "â€? : self.gatewayManager.log)
                            .font(.caption.monospaced())
                            .frame(maxWidth: .infinity, alignment: .leading)
                            .textSelection(.enabled)
                    }
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Toggle("Write rolling diagnostics log (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            ...
                            .help(
                                "Writes a rotating, local-only log under ~/Library/Logs/openx/. " +
    ...
    private var pathsSection: some View {
        GroupBox("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("openx project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to openx repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/openx").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.clawdbot/openx.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            HStack(spacing: 8) {
                                ...
                                Button {
                                    ...
                                    Label(
                                        self.modelsLoading ? "Reloadingâ€? : "Reload models",
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Button {
                        ...
                        Label(
                            self.debugSendInFlight ? "Sending debug voiceâ€? : "Send debug voice",
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting openx after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startopenx()
                    } label: {
                        Label("Restart openx", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
        ...
        GroupBox("Canvas") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                if let canvasSnapshotPath {
                    HStack(spacing: 8) {
                        Text("snapshot â†?\(canvasSnapshotPath)")
                            .font(.caption2.monospaced())
                            .foregroundStyle(.secondary)
                            .lineLimit(1)
                            .truncationMode(.middle)
                ...
                if let canvasError {
                    ...
                    Text("Tip: the session directory is returned by â€œShow panelâ€?")
                        .font(.caption2)
                        .foregroundStyle(.tertiary)
                }
            }
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/openx"
