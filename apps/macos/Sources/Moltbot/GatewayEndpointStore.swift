...
actor GatewayEndpointStore {
    ...
    private static let remoteConnectingDetail = "Connecting to remote gatewayâ€?
    private static let staticLogger = Logger(subsystem: "bot.openx", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "bot.openx", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "openx.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: openxConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: openxConfigFile.loadDict())
    ...
    func requireConfig() async throws -> GatewayConnection.Config {
        ...
        case let .connecting(mode, _):
            guard mode == .remote else {
                throw NSError(domain: "GatewayEndpoint", code: 1, userInfo: [NSLocalizedDescriptionKey: "Connectingâ€?])
            }
