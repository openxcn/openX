...
struct Semver: Comparable, CustomStringConvertible, Sendable {
    ...
    static func parse(_ raw: String?) -> Semver? {
        ...
        // Strip prerelease suffix (e.g., "11-4" â†?"11", "5-beta.1" â†?"5")
...
struct GatewayEnvironmentStatus: Equatable {
    ...
    static var checking: Self {
        .init(kind: .checking, nodeVersion: nil, gatewayVersion: nil, requiredGateway: nil, message: "Checkingâ€?)
    }
}
...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "bot.openx", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = openxConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.openxExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "openx CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "openx@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "openx@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "openx@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "openx@\(target)"])
            }

        statusHandler("Installing openx@\(target) via \(label)â€?)
        ...
        if response.success {
            statusHandler("Installed openx@\(target)")
        } else {
