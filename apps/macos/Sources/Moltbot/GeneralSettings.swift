        ...
        ScrollView(.vertical) {
            VStack(alignment: .leading, spacing: 18) {
                VStack(alignment: .leading, spacing: 12) {
                    SettingsToggleRow(
                        title: "openx active",
                        subtitle: "Pause to stop the openx gateway; no messages will be processed.",
                    ...
                    SettingsToggleRow(
                        ...
                        subtitle: "Automatically start openx after you sign in.",
                        binding: self.$state.launchAtLogin)

                    SettingsToggleRow(
                        title: "Show Dock icon",
                        subtitle: "Keep openx visible in the Dock instead of menu-bar-only mode.",
                ...
                HStack {
                    Spacer()
                    Button("Quit openx") { NSApp.terminate(nil) }
        ...
        VStack(alignment: .leading, spacing: 10) {
                ...
                DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        LabeledContent("CLI path") {
                            TextField("/Applications/openx.app/.../openx", text: self.$state.remoteCliPath)
                                .textFieldStyle(.roundedBorder)
                                .frame(width: 280)
                        }
                    }
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if !self.isControlStatusDuplicate || ControlChannel.shared.lastPingMs != nil {
                    ...
                    let line = [status, ping].compactMap(\.self).joined(separator: " Â· ")
                    if !line.isEmpty {
                        Text(line)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                ...
                if let hb = HeartbeatStore.shared.lastEvent {
                    ...
                    Text("Last heartbeat: \(hb.status) Â· \(ageText)")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for openx logs in /tmp/openx/.
    ...
    private func applyDiscoveredGateway(_ gateway: GatewayDiscoveryModel.DiscoveredGateway) {
        ...
        if self.state.remoteTransport == .direct {
            ...
            openxConfigFile.setRemoteGatewayUrl(host: host, port: gateway.gatewayPort)
        }
    }
}
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/openx"
        state.remoteCliPath = "/tmp/openx"
