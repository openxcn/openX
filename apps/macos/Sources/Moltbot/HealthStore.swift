...
final class HealthStore {
    ...
    private static let logger = Logger(subsystem: "bot.openx", category: "health")
    ...
    var summaryLine: String {
        if self.isRefreshing { return "Health check running鈥? }
        ...
        if link.summary.linked != true {
            if let fallback = self.resolveFallbackChannel(snap, excluding: link.id) {
                ...
                return "\(fallbackLabel) \(fallbackState) 路 Not linked 鈥?run openx login"
            }
            return "Not linked 鈥?run openx login"
        }
        let auth = link.summary.authAgeMs.map { msToAge($0) } ?? "unknown"
        if let probe = link.summary.probe, probe.ok == false {
            let status = probe.status.map(String.init) ?? "?"
            let suffix = probe.status == nil ? "probe degraded" : "probe degraded 路 status \(status)"
            return "linked 路 auth \(auth) 路 \(suffix)"
        }
        return "linked 路 auth \(auth)"
    }
    ...
    var detailLine: String? {
        if let error = self.lastError, !error.isEmpty {
            ...
            if lower.contains("connection refused") {
                ...
                return "The gateway control port (\(host)) isn鈥檛 listening 鈥?restart openx to bring it back."
            }
