...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/openx")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("openx-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("openx-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/openx/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("openx") && $0.pathExtension == "log" }
            .max { lhs, rhs in
