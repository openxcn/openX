...
enum openxLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = openxOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = openxFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        openxLogging.bootstrapIfNeeded()
        let label = openxLogging.makeLabel(subsystem: subsystem, category: category)
...
struct openxOSLogHandler: LogHandler {
...
struct openxFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = openxLogging.parseLabel(self.label)
