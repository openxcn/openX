...
/// Menu contents for the openx menu bar extra.
struct MenuContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Toggle(isOn: self.activeBinding) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if self.pairingPrompter.pendingCount > 0 {
                        ...
                        let repairSuffix = repairCount > 0 ? " 路 \(repairCount) repair" : ""
                        self.statusLine(
            ...
            .opacity(voiceWakeSupported ? 1 : 0.5)
            Divider()
            Button("Settings鈥?) { self.open(tab: .general) }
                .keyboardShortcut(",", modifiers: [.command])
            self.debugMenu
            Button("About openx") { self.open(tab: .about) }
        ...
        if self.state.debugPaneEnabled {
            Menu("Debug") {
                ...
                Button {
                    ...
                    Label("Open Agent Events鈥?, systemImage: "bolt.horizontal.circle")
                }
                Button {
    ...
    private func open(tab: SettingsTab) {
        ...
        DispatchQueue.main.async {
            NotificationCenter.default.post(name: .openxSelectSettingsTab, object: tab)
        }
    }
    ...
    private var healthStatus: (label: String, color: Color) {
        if let activity = self.activityStore.current {
            ...
            let text = "\(roleLabel) 路 \(activity.label)"
            return (text, color)
        }
        ...
        if isRefreshing {
            return ("Health check running鈥?, health.tint)
        }

        switch health {
        case .ok:
            let ageText = lastAge.map { " 路 checked \($0)" } ?? ""
    ...
    private var heartbeatStatus: (label: String, color: Color) {
        if case .degraded = self.controlChannel.state {
            ...
            switch evt.status {
            case "sent":
                return ("Last heartbeat sent 路 \(ageText)", .blue)
            case "ok-empty", "ok-token":
                return ("Heartbeat ok 路 \(ageText)", .green)
            case "skipped":
                return ("Heartbeat skipped 路 \(ageText)", .secondary)
            case "failed":
                return ("Heartbeat failed 路 \(ageText)", .red)
            default:
                return ("Heartbeat 路 \(ageText)", .secondary)
            }
        } else {
    ...
    private var voiceWakeMicMenu: some View {
        Menu {
            ...
            if self.loadingMics {
                Divider()
                Label("Refreshing microphones鈥?, systemImage: "arrow.triangle.2.circlepath")
