...
extension MenuSessionsInjector {
    ...
    private func inject(into menu: NSMenu) {
        ...
        if let snapshot = self.cachedSnapshot {
            ...
            let statusText = isConnected
                ? (self.cachedErrorText ?? "Loading sessionsâ€?)
    ...
    private func injectNodes(into menu: NSMenu) {
        ...
        if case .connecting = ControlChannel.shared.state {
            menu.insertItem(
                self.makeMessageItem(text: "Connectingâ€?, symbolName: "circle.dashed", width: width),
                at: cursor)
            cursor += 1
            return
        }
...
extension MenuSessionsInjector {
    ...
    private func compactUsageError(_ error: Error) -> String {
        ...
        if message.count > 90 { return "\(message.prefix(87))â€? }
        return message
    }
...
extension MenuSessionsInjector {
    ...
    private func buildSubmenu(for row: SessionRow, storePath: String) -> NSMenu {
        ...
        let morePreview = NSMenuItem(title: "More previewâ€?, action: nil, keyEquivalent: "")
    ...
    private func resetSession(_ sender: NSMenuItem) {
        ...
        Task { @MainActor in
            guard SessionActions.confirmDestructiveAction(
                title: "Reset session?",
                message: "Starts a new session id for â€œ\(key)â€?",
                action: "Reset")
            else { return }
    ...
    private func deleteSession(_ sender: NSMenuItem) {
        ...
        Task { @MainActor in
            guard SessionActions.confirmDestructiveAction(
                ...
                message: "Deletes the â€œ\(key)â€?entry and archives its transcript.",
                action: "Delete")
            else { return }
