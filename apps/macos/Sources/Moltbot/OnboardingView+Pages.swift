    ...
    func welcomePage() -> some View {
        self.onboardingPage {
            VStack(spacing: 22) {
                ...
                Text("openx is a powerful personal AI assistant that can connect to WhatsApp or Telegram.")
                    .font(.body)
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            Text(
                                "The connected AI agent (e.g. Claude) can trigger powerful actions on your Mac, " +
                                    ...
                                    "Only enable openx if you understand the risks and trust the prompts and " +
    ...
    func connectionPage() -> some View {
        self.onboardingPage {
            Text("Choose your Gateway")
                .font(.largeTitle.weight(.semibold))
            Text(
                "openx uses a single Gateway that stays running. Pick this Mac, " +
            ...
            self.onboardingCard(spacing: 12, padding: 14) {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if self.gatewayDiscovery.gateways.isEmpty {
                        Text("Searching for nearby gatewaysâ€?)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                            .padding(.leading, 4)
                    } else {
                                ...
                                if self.state.remoteTransport == .ssh {
                                    ...
                                    GridRow {
                                        Text("Project root")
                                            .font(.callout.weight(.semibold))
                                            .frame(width: labelWidth, alignment: .leading)
                                        TextField("/home/you/Projects/openx", text: self.$state.remoteProjectRoot)
                                            .textFieldStyle(.roundedBorder)
                                            .frame(width: fieldWidth)
                                    }
                                    GridRow {
    ...
    func anthropicAuthPage() -> some View {
        self.onboardingPage {
            ...
            Text("Give your model the token it needs!")
                ...
                .frame(maxWidth: 540)
                .fixedSize(horizontal: false, vertical: true)
            Text("openx supports any model â€?we strongly recommend Opus 4.5 for the best experience.")
                .font(.callout)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 540)
            ...
            self.onboardingCard(spacing: 12, padding: 16) {
                    ...
                    Text(
                        self.anthropicAuthConnected
                            ? (self.anthropicAuthVerified
                                ? "Claude connected (OAuth) â€?verified"
                ...
                Text(
                    "This lets openx use Claude immediately. Credentials are stored at " +
                ...
                HStack(spacing: 12) {
                    Text(openxOAuthStore.oauthURL().path)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .lineLimit(1)
                        .truncationMode(.middle)
                    ...
                    Button("Reveal") {
                        NSWorkspace.shared.activateFileViewerSelecting([openxOAuthStore.oauthURL()])
                    }
    ...
    func permissionsPage() -> some View {
        self.onboardingPage {
            Text("Grant permissions")
                .font(.largeTitle.weight(.semibold))
            Text("These macOS permissions let openx automate apps and capture context on this Mac.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func cliPage() -> some View {
        self.onboardingPage {
            Text("Install the CLI")
                .font(.largeTitle.weight(.semibold))
            Text("Required for local mode: installs `openx` so launchd can run the gateway.")
                .font(.body)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
                .frame(maxWidth: 520)
    ...
    func workspacePage() -> some View {
        self.onboardingPage {
            Text("Agent workspace")
                .font(.largeTitle.weight(.semibold))
            Text(
                "openx runs the agent from a dedicated workspace so it can load `AGENTS.md` " +
                ...
                if self.state.connectionMode == .remote {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Workspace folder")
                            .font(.headline)
                        TextField(
                            AgentWorkspace.displayPath(for: openxConfigFile.defaultWorkspaceURL()),
                                ...
                                Task {
                                    ...
                                    if saved {
                                        self.workspaceStatus =
                                            "Saved to ~/.clawdbot/openx.json (agents.defaults.workspace)"
                                    }
                                }
                            }
                            .buttonStyle(.bordered)
    ...
    func onboardingChatPage() -> some View {
        VStack(spacing: 16) {
            ...
            self.onboardingGlassCard(padding: 8) {
                openxChatView(viewModel: self.onboardingChatModel, style: .onboarding)
            ...
            self.onboardingCard {
                ...
                if self.state.connectionMode == .remote {
                    self.featureRow(
                        ...
                        subtitle: """
                        On your gateway host: install/update the `openx` package and make sure credentials exist
                ...
                self.featureRow(
                    ...
                    subtitle: "Click the openx menu bar icon for quick chat and status.",
                    systemImage: "bubble.left.and.bubble.right")
                self.featureActionRow(
                    title: "Connect WhatsApp or Telegram",
                    subtitle: "Open Settings â†?Channels to link channels and monitor status.",
                    systemImage: "link",
                    buttonTitle: "Open Settings â†?Channels")
                {
                ...
                self.featureActionRow(
                    ...
                    buttonTitle: "Open Settings â†?Skills")
                {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let error = self.onboardingSkillsModel.error {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Couldnâ€™t load skills from the Gateway.")
                        .font(.footnote.weight(.semibold))
                        .foregroundStyle(.orange)
                    Text(
                        "Make sure the Gateway is running and connected, " +
                            "then hit Refresh (or open Settings â†?Skills).")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)
                    Text("Details: \(error)")
