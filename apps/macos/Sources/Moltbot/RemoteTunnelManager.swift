...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "bot.openx", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous openx run already has an SSH listener on the expected port (common after restarts),
