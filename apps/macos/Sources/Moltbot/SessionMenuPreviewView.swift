...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "bot.openx", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> openxSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: openxSessionPreviewEntry,
    ...
    private static func cache(payload: openxSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: openxChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [openxKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [openxKit.AnyCodable]) -> [openxChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(openxChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: openxChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: openxChatMessage) -> Bool {
    ...
    private static func toolNames(for message: openxChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: openxChatMessage) -> String? {
