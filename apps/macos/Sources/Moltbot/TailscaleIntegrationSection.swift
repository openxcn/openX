...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawdbot/openx.json. Prefer CLAWDBOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawdbot/openx.json. Restarting gatewayâ€?
        } else {
            self.statusMessage = "Saved to ~/.clawdbot/openx.json. Restart the gateway to apply."
        }
