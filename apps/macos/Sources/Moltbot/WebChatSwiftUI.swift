...
private let webChatSwiftLogger = Logger(subsystem: "bot.openx", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: openxChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> openxChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(openxChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> openxChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                openxGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<openxChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any openxChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = openxChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: openxChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<openxChatView>) -> NSViewController
    {
