...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "bot.openx", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in openxBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: openxBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "bot.openx.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        let domain = openxBonjour.wideAreaGatewayServiceDomain
    ...
    private func updateGateways(for domain: String) {
        ...
        if domain == openxBonjour.wideAreaGatewayServiceDomain,
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (openx)", with: "")
