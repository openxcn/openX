...
struct openxMacCLI {
    static func main() async {
        ...
        default:
            fputs("openx-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      openx-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      openx-mac discover [--timeout <ms>] [--json] [--include-local]
      openx-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      openx-mac connect
      openx-mac connect --url ws://127.0.0.1:18789 --json
      openx-mac discover --timeout 3000 --json
      openx-mac wizard --mode local
    """)
}
