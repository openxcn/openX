...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: openxProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: openxProtocol.AnyCodable] = [
            "presence": .init([openxProtocol.AnyCodable(entry)]),
        ]
        let payload = openxProtocol.AnyCodable(payloadMap)
