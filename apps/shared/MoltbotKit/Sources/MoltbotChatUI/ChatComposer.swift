...
@MainActor
struct openxChatComposer: View {
    @Bindable var viewModel: openxChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(openxChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(openxChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \openxPendingAttachment.id)
                { (att: openxPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            openxPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(openxChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(openxChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(openxChatTheme.subtleCard)
        .clipShape(Capsule())
    }
