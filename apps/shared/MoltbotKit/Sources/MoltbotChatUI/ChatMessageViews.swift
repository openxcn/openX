...
struct ChatMessageBubble: View {
    let message: openxChatMessage
    let style: openxChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? openxChatTheme.userText : openxChatTheme.assistantText
    ...
    private var inlineAttachments: [openxChatMessageContent] {
    ...
    private var toolCalls: [openxChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [openxChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? openxChatTheme.userBubble
        }
        if self.style == .onboarding {
            return openxChatTheme.onboardingAssistantBubble
        }
        return openxChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return openxChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: openxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? openxChatTheme.userText : openxChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: openxChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(openxChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(openxChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [openxChatPendingToolCall]

    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Label("Running toolsâ€?, systemImage: "hammer")
                .font(.caption)
                .foregroundStyle(.secondary)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: openxChatTheme.assistantText)
            }
        }
    }
}
