...
private let chatUILogger = Logger(subsystem: "bot.openx", category: "openxChatUI")

@MainActor
@Observable
public final class openxChatViewModel {
    public private(set) var messages: [openxChatMessage] = []
    ...
    public var attachments: [openxPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [openxChatPendingToolCall] = []
    public private(set) var sessions: [openxChatSessionEntry] = []
    private let transport: any openxChatTransport
    ...
    private var pendingToolCallsById: [String: openxChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any openxChatTransport) {
    ...
    public var sessionChoices: [openxChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [openxChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [openxChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: openxPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [openxChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: openxChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [openxChatMessage]) -> [openxChatMessage] {
        var result: [openxChatMessage] = []
    ...
    private static func dedupeKey(for message: openxChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [openxChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> openxChatAttachmentPayload in
            openxChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> openxChatSessionEntry {
        openxChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: openxChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: openxChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: openxAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = openxChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> openxPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
