...
private final class TestChatTransport: @unchecked Sendable, openxChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [openxChatHistoryPayload]
    private let sessionsResponses: [openxChatSessionsListResponse]

    private let stream: AsyncStream<openxChatTransportEvent>
    private let continuation: AsyncStream<openxChatTransportEvent>.Continuation

    init(
        historyResponses: [openxChatHistoryPayload],
        sessionsResponses: [openxChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<openxChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> openxChatHistoryPayload {
        ...
        return self.historyResponses.last ?? openxChatHistoryPayload(
        ...
        attachments _: [openxChatAttachmentPayload]) async throws -> openxChatSendResponse
    {
        ...
        return openxChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> openxChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? openxChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: openxChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = openxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = openxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { openxChatViewModel(sessionKey: "main", transport: transport) }
        ...
        transport.emit(
            .agent(
                openxAgentEventPayload(
                    runId: sessionId,
                    seq: 1,
                    stream: "assistant",
                    ts: Int(Date().timeIntervalSince1970 * 1000),
                    data: ["text": AnyCodable("streamingâ€?)])))

        try await waitUntil("assistant stream visible") {
            await MainActor.run { vm.streamingAssistantText == "streamingâ€? }
        }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = openxChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = openxChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = openxChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { openxChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
