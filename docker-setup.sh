...
IMAGE_NAME="${CLAWDBOT_IMAGE:-openx:local}"
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm openx-cli onboard --no-install-daemon

echo ""
echo "==> Provider setup (optional)"
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm openx-cli providers login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm openx-cli providers add --provider telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm openx-cli providers add --provider discord --token <token>"
echo "Docs: https://docs.openx.bot/providers"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d openx-gateway
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f openx-gateway"
echo "  ${COMPOSE_HINT} exec openx-gateway node dist/index.js health --token \"$CLAWDBOT_GATEWAY_TOKEN\""
