...
This document describes how the openx ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`openx acp` exposes an ACP agent over stdio and forwards prompts to a running
openx Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a openx Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `openx acp` over stdio.

Example config:

```bash
openx config set gateway.remote.url wss://gateway-host:18789
openx config set gateway.remote.token <token>
```

Example run:

```bash
openx acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
openx acp --session agent:main:main
openx acp --session agent:design:main
openx acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select "openx ACP" to start a thread.

## Execution Model

- ACP client spawns `openx acp` and speaks ACP messages over stdio.
...
```bash
openx acp --session agent:main:main
openx acp --session-label "support inbox"
openx acp --reset-session
```
...
`openx acp` resolves the Gateway URL and auth from CLI flags or config:
