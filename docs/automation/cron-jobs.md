...
- Jobs persist under `~/.clawdbot/cron/` so restarts donâ€™t lose schedules.
...
If `channel` or `to` is omitted, cron can fall back to the main sessionâ€™s â€œlast routeâ€?(the last place the agent replied).

Delivery notes:
- If `to` is set, cron auto-delivers the agentâ€™s final output even if `deliver` is omitted.
...
openx cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
openx cron edit <jobId> --agent ops
openx cron edit <jobId> --clear-agent
```
```
...
```bash
openx system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`openx system event`](/cli/system).

## Troubleshooting

### â€œNothing runsâ€?- Check cron is enabled: `cron.enabled` and `CLAWDBOT_SKIP_CRON`.
