---
summary: "Gmail Pub/Sub push wired into openx webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to openx
  - Setting up Pub/Sub push for agent wake
---

# Gmail Pub/Sub -> openx

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> openx webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- openx hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the openx helper to wire everything together (installs deps on macOS via brew):

```bash
openx webhooks gmail setup \
  --account openx@gmail.com
```

Defaults:
- Uses Tailscale Funnel for the public push endpoint.
- Writes `hooks.gmail` config for `openx webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, openx automatically sets
...
- `--hook-url` points to openx `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to openx.

Recommended: `openx webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account openx@gmail.com
gog gmail history --account openx@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account openx@gmail.com
