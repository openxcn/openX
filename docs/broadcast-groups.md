...
Broadcast groups are evaluated after channel allowlists and group activation rules. In WhatsApp groups, this means broadcasts happen when openx would normally reply (for example: on mention, depending on your group settings).
...
**Result:** When openx would reply in this chat, it will run all three agents.
...
‚ú?**Good:** Each agent has one job  
‚ù?**Bad:** One generic "dev-helper" agent
