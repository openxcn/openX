...
Matrix is an open, decentralized messaging protocol. openx connects as a Matrix **user**
...
```bash
openx plugins install @openx/matrix
```

Local checkout (when running from a git repo):

```bash
openx plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `openx plugins install @openx/matrix`
   - From a local checkout: `openx plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: openx calls the same
...
- On first connection, openx requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  openx logs a warning.
...
- Approve via:
  - `openx pairing list matrix`
  - `openx pairing approve matrix <CODE>`
...
- On startup, openx resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
