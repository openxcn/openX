...
```bash
openx plugins install @openx/msteams
```

Local checkout (when running from a git repo):
```bash
openx plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
openx will offer the local install path automatically.
...
3) Configure openx with those credentials.
...
## Goals
- Talk to openx via Teams DMs, group chats, or channels.
...
- On startup, openx resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.clawdbot/openx.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring openx, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `openx-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `openx plugins install @openx/msteams`
   - From a local checkout: `openx plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "openx in Teams", "full": "openx in Teams" },
...
openx handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, openx only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/openxShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
openx sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `openx message poll --channel msteams --target conversation:<id> ...`
...
**CLI:**
```bash
openx message send --channel msteams \
...
openx message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
openx message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
openx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
openx message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
...
https://teams.microsoft.com/l/team/19%3ABk4j...%40thread.tacv2/conversations?groupId=...
                                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”?                                    Team ID (URL-decode this)
```
...
- **"Something went wrong" on upload:** Upload via https://admin.teams.microsoft.com instead, open browser DevTools (F12) â†?Network tab, and check the response body for the actual error.
