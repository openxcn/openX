...
```bash
openx plugins install @openx/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
openx plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "openx" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `openx pairing list nextcloud-talk`
  - `openx pairing approve nextcloud-talk <CODE>`
