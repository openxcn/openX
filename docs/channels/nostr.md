...
read_when:
  - You want openx to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables openx to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`openx onboard`) and `openx channels add` list optional channel plugins.
...
```bash
openx plugins install @openx/nostr
```

Use a local checkout (dev workflows):

```bash
openx plugins install --link <path-to-openx>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "openx@example.com",
        "lud16": "openx@example.com"
      }
    }
  }
}
