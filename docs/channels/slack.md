...
3) Set tokens for openx and start the gateway.
...
### User token (optional)
openx can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, openx falls back to the user token.
...
```json
{
  "display_information": {
    "name": "openx",
    "description": "Slack connector for openx"
  },
  "features": {
    "bot_user": {
      "display_name": "openx",
    ...
    "slash_commands": [
      {
        "command": "/clawd",
        "description": "Send a message to openx",
        "should_escape": false
      }
    ]
  },
...
## Reply threading
By default, openx replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesnâ€™t provide `channel_type`, openx infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `openx pairing approve slack <code>`.
...
- `allowlist` requires channels to be listed in `channels.slack.channels`.
 ...
 - On startup, openx resolves channel/user names in allowlists to IDs (when tokens allow)
