...
Tlon is a decentralized messenger built on Urbit. openx connects to your Urbit ship and can
...
```bash
openx plugins install @openx/tlon
```

Local checkout (when running from a git repo):

```bash
openx plugins install ./extensions/tlon
```
...
Use these with `openx message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, openx replies in-thread.
