...
read_when:
  - Setting up Twitch chat integration for openx
---
# Twitch (plugin)

Twitch chat support via IRC connection. openx connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
openx plugins install @openx/twitch
```

Local checkout (when running from a git repo):

```bash
openx plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "openx",              // Bot's Twitch account
