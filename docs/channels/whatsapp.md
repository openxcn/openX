...
2) Configure WhatsApp in `~/.clawdbot/openx.json`.
3) Run `openx channels login` to scan the QR code (Linked Devices).
4) Start the gateway.
...
WhatsApp requires a real mobile number for verification. VoIP and virtual numbers are usually blocked. There are two supported ways to run openx on WhatsApp:

### Dedicated number (recommended)
Use a **separate phone number** for openx. Best UX, clean routing, no self-chat quirks. Ideal setup: **spare/old Android phone + eSIM**. Leave it on Wi‚ÄëFi and power, and link it via QR.

**WhatsApp Business:** You can use WhatsApp Business on the same device with a different number. Great for keeping your personal WhatsApp separate ‚Ä?install WhatsApp Business and register the openx number there.
...
Quick fallback: run openx on **your own number**. Message yourself (WhatsApp ‚ÄúMessage yourself‚Ä? for testing so you don‚Äôt spam contacts. Expect to read verification codes on your main phone during setup and experiments. **Must enable self-chat mode.**
...
Self-chat replies default to `[{identity.name}]` when set (otherwise `[openx]`)
...
- **Local eSIM** from your country's mobile carrier (most reliable)
  ...
  - UK: [giffgaff](https://www.giffgaff.com) ‚Ä?free SIM, no contract
- **Prepaid SIM** ‚Ä?cheap, just needs to receive one SMS for verification

**Avoid:** TextNow, Google Voice, most "free SMS" services ‚Ä?WhatsApp blocks these aggressively.

**Tip:** The number only needs to receive one verification SMS. After that, WhatsApp Web sessions persist via `creds.json`.

## Why Not Twilio?
- Early openx builds supported Twilio‚Äôs WhatsApp Business integration.
...
- High-volume or ‚Äúchatty‚Ä?usage triggers aggressive blocking, because business accounts aren‚Äôt meant to send dozens of personal assistant messages.
- Result: unreliable delivery and frequent blocks, so support was removed.

## Login + credentials
- Login command: `openx channels login` (QR via Linked Devices).
- Multi-account login: `openx channels login --account <id>` (`<id>` = `accountId`).
...
- Logout: `openx channels logout` (or `--account <id>`) deletes WhatsApp auth state (but keeps shared `oauth.json`).
...
- **DM policy**: `channels.whatsapp.dmPolicy` controls direct chat access (default: `pairing`).
  - Pairing: unknown senders get a pairing code (approve via `openx pairing approve whatsapp <code>`; codes expire after 1 hour).
...
If you run openx on your **personal WhatsApp number**, enable `channels.whatsapp.selfChatMode` (see sample above).
...
**Will openx message random contacts when I link WhatsApp?**  
No. Default DM policy is **pairing**, so unknown senders only get a pairing code and their message is **not processed**. openx only replies to chats it receives, or to sends you explicitly trigger (agent/CLI).

**How does pairing work on WhatsApp?**  
Pairing is a DM gate for unknown senders:
- First DM from a new sender returns a short code (message is not processed).
- Approve with: `openx pairing approve whatsapp <code>` (list with `openx pairing list whatsapp`).
- Codes expire after 1 hour; pending requests are capped at 3 per channel.

**Can multiple people use different openxs on one WhatsApp number?**  
...
**Options:**
- `emoji` (string): Emoji to use for acknowledgment (e.g., "üëÄ", "‚ú?, "üì®"). Empty or omitted = feature disabled.
...
- Media:
  ...
  - Animated GIFs: WhatsApp expects MP4 with `gifPlayback: true` for inline looping.
    - CLI: `openx message send --media <mp4> --gif-playback`
    - Gateway: `send` params include `gifPlayback: true`

## Voice notes (PTT audio)
WhatsApp sends audio as **voice notes** (PTT bubble).
- Best results: OGG/Opus. openx rewrites `audio/ogg` to `audio/ogg; codecs=opus`.
...
- Log file: `/tmp/openx/openx-YYYY-MM-DD.log` (configurable).
- Troubleshooting guide: [Gateway troubleshooting](/gateway/troubleshooting).

## Troubleshooting (quick)

**Not linked / QR login required**
- Symptom: `channels status` shows `linked: false` or warns ‚ÄúNot linked‚Ä?
- Fix: run `openx channels login` on the gateway host and scan the QR (WhatsApp ‚Ü?Settings ‚Ü?Linked Devices).

**Linked but disconnected / reconnect loop**
- Symptom: `channels status` shows `running, disconnected` or warns ‚ÄúLinked but disconnected‚Ä?
- Fix: `openx doctor` (or restart the gateway). If it persists, relink via `channels login` and inspect `openx logs --follow`.
