...
- Install via CLI: `openx plugins install @openx/zalo`
- Or select **Zalo** during onboarding and confirm the install prompt
- Details: [Plugins](/plugin)

## Quick setup (beginner)
1) Install the Zalo plugin:
   - From a source checkout: `openx plugins install ./extensions/zalo`
   - From npm (if published): `openx plugins install @openx/zalo`
...
- Approve via:
  - `openx pairing list zalo`
  - `openx pairing approve zalo <CODE>`
...
| Feature | Status |
|---------|--------|
| Direct messages | ‚ú?Supported |
| Groups | ‚ù?Coming soon (per Zalo docs) |
| Media (images) | ‚ú?Supported |
| Reactions | ‚ù?Not supported |
| Threads | ‚ù?Not supported |
| Polls | ‚ù?Not supported |
| Native commands | ‚ù?Not supported |
| Streaming | ‚ö†Ô∏è Blocked (2000 char limit) |

## Delivery targets (CLI/cron)
- Use a chat id as the target.
- Example: `openx message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**
- Check that the token is valid: `openx channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `openx logs --follow`
