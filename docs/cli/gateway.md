---
summary: "openx Gateway CLI (`openx gateway`) â€?run, query, and discover gateways"
read_when:
...
The Gateway is openxâ€™s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `openx gateway â€¦`.
...
Notes:
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.clawdbot/openx.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
openx gateway health --url ws://127.0.0.1:18789
```
...
```bash
openx gateway probe --ssh user@gateway-host
```
...
```bash
openx gateway call status
openx gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
`gateway discover` scans for Gateway beacons (`_openx-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): `openx.internal.` (requires split DNS + DNS server; see [/gateway/bonjour](/gateway/bonjour))
...
```bash
openx gateway discover --timeout 4000
openx gateway discover --json | jq '.beacons[].wsUrl'
```
