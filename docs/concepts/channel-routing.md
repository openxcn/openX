...
openx routes replies **back to the channel where a message came from**. The
...
Broadcast groups let you run **multiple agents** for the same peer **when openx would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
