...
read_when:
  - You want to understand what â€œcontextâ€?means in openx
  - You are debugging why the model â€œknowsâ€?something (or forgot it)
  - You want to reduce context overhead (/context, /status, /compact)
---
# Context

â€œContextâ€?is **everything openx sends to the model for a run**. It is bounded by the modelâ€™s **context window** (token limit).

Beginner mental model:
- **System prompt** (openx-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
Tools: read, edit, write, exec, process, browser, message, sessions_send, â€?Tool list (system prompt text): 1,032 chars (~258 tok)
...
â€?Top skills (prompt entry size):
...
## How openx builds the system prompt

The system prompt is **openx-owned** and rebuilt each run. It includes:
...
By default, openx injects a fixed set of workspace files (if present):
- `AGENTS.md`
- `SOUL.md`
- `TOOLS.md`
- `IDENTITY.md`
