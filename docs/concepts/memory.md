---
summary: "How openx memory works (workspace files + automatic memory flush)"
read_when:
...
openx memory is **plain Markdown in the agent workspace**. The files are the
...
When a session is **close to auto-compaction**, openx triggers a **silent,
...
openx can build a small vector index over `MEMORY.md` and `memory/*.md` (plus
...
- Uses remote embeddings by default. If `memorySearch.provider` is not set, openx auto-selects:
...
Remote embeddings **require** an API key for the embedding provider. openx
...
- Reindex triggers: the index stores the embedding **provider/model + endpoint fingerprint + chunking params**. If any of those change, openx automatically resets and reindexes the entire store.

### Hybrid search (BM25 + vector)

When enabled, openx combines:
- **Vector similarity** (semantic match, wording can differ)
- **BM25 keyword relevance** (exact tokens like IDs, env vars, code symbols)

If full-text search is unavailable on your platform, openx falls back to vector-only search.

#### Why hybrid?

Vector search is great at â€œthis means the same thingâ€?
- â€œMac Studio gateway hostâ€?vs â€œthe machine running the gatewayâ€?- â€œdebounce file updatesâ€?vs â€œavoid indexing on every writeâ€?
...
openx can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, openx stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, openx logs the
