...
- `/usage cost` in chats: local cost summary aggregated from openx session logs.
- CLI: `openx status --usage` prints a full per-provider breakdown.
- CLI: `openx channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
