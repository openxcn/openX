---
summary: "Where openx loads environment variables and the precedence order"
read_when:
...
openx pulls environment variables from multiple sources. The rule is **never override existing values**.
...
4) **Config `env` block** in `~/.clawdbot/openx.json` (applied only if missing).
