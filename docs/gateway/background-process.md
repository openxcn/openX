...
openx runs shell commands through the `exec` tool and keeps long‑running tasks in memory. The `process` tool manages those background sessions.
...
- `PI_BASH_JOB_TTL_MS`: TTL for finished sessions (ms, bounded to 1m�?h)
