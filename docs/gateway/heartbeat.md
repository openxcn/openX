...
- During heartbeat runs, openx treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `openx sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, openx skips the heartbeat run entirely (no model call).
...
- â€œUpdate `HEARTBEAT.md` to add a daily calendar check.â€?- â€œRewrite `HEARTBEAT.md` so itâ€™s shorter and focused on inbox follow-ups.â€?
...
```bash
openx system event --text "Check for urgent follow-ups" --mode now
```
