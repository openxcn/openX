...
- Replaces the legacy `gateway` command. CLI entry point: `openx gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.clawdbot/openx.json` (or `CLAWDBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__openx__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
- macOS: `bot.openx.<profile>` (legacy `com.clawdbot.*` may still exist)
- Linux: `openx-gateway-<profile>.service`
- Windows: `openx Gateway (<profile>)`

Install metadata is embedded in the service config:
- `CLAWDBOT_SERVICE_MARKER=openx`
...
```bash
openx --dev setup
openx --dev gateway --allow-unconfigured
...
- `CLAWDBOT_CONFIG_PATH=~/.clawdbot-dev/openx.json`
...
Example:
```bash
CLAWDBOT_CONFIG_PATH=~/.clawdbot/a.json CLAWDBOT_STATE_DIR=~/.clawdbot-a openx gateway --port 19001
CLAWDBOT_CONFIG_PATH=~/.clawdbot/b.json CLAWDBOT_STATE_DIR=~/.clawdbot-b openx gateway --port 19002
```
...
## Methods (initial set)
- `health` â€?full health snapshot (same shape as `openx health --json`).
- `status` â€?short summary.
- `system-presence` â€?current presence list.
- `system-event` â€?post a presence/system note (structured).
- `send` â€?send a message via the active channel(s).
- `agent` â€?run an agent turn (streams events back on same connection).
- `node.list` â€?list paired + currently-connected nodes (includes `caps`, `deviceFamily`, `modelIdentifier`, `paired`, `connected`, and advertised `commands`).
- `node.describe` â€?describe a node (capabilities + supported `node.invoke` commands; works for paired nodes and for currently-connected unpaired nodes).
- `node.invoke` â€?invoke a command on a node (e.g. `canvas.*`, `camera.*`).
- `node.pair.*` â€?pairing lifecycle (`request`, `list`, `approve`, `reject`, `verify`).

See also: [Presence](/concepts/presence) for how presence is produced/deduped and why a stable `client.instanceId` matters.

## Events
- `agent` â€?streamed tool/output events from the agent run (seq-tagged).
- `presence` â€?presence updates (deltas with stateVersion) pushed to all connected clients.
- `tick` â€?periodic keepalive/no-op to confirm liveness.
- `shutdown` â€?Gateway is exiting; payload includes `reason` and optional `restartExpectedMs`. Clients should reconnect.
...
- Clients (TS/Swift) consume generated types (TS directly; Swift via the repoâ€™s generator).
...
- Standard codes:
  - `NOT_LINKED` â€?WhatsApp not authenticated.
  - `AGENT_TIMEOUT` â€?agent did not respond within the configured deadline.
  - `INVALID_REQUEST` â€?schema/param validation failed.
  - `UNAVAILABLE` â€?Gateway is shutting down or a dependency is unavailable.
...
- Use launchd to keep the service alive:
  - Program: path to `openx`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `openx gateway install` writes `~/Library/LaunchAgents/bot.openx.gateway.plist`
    (or `bot.openx.<profile>.plist`; legacy `com.clawdbot.*` is cleaned up).
  - `openx doctor` audits the LaunchAgent config and can update it to current defaults.
...
Notes:
- `gateway status` probes the Gateway RPC by default using the serviceâ€™s resolved port/config (override with `--url`).
...
- `gateway status` prints config path + probe target to avoid â€œlocalhost vs LAN bindâ€?confusion and profile mismatches.
- `gateway status` includes the last gateway error line when the service looks running but the port is closed.
- `logs` tails the Gateway file log via RPC (no manual `tail`/`grep` needed).
- If other gateway-like services are detected, the CLI warns unless they are openx profile services.
  ...
  - Cleanup: `openx gateway uninstall` (current service) and `openx doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `openx gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- openx.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.openx.gateway` (or `bot.openx.<profile>`; legacy `com.clawdbot.*` labels still unload cleanly).
- To stop it cleanly, use `openx gateway stop` (or `launchctl bootout gui/$UID/bot.openx.gateway`).
- To restart, use `openx gateway restart` (or `launchctl kickstart -k gui/$UID/bot.openx.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `openx gateway install` first.
  - Replace the label with `bot.openx.<profile>` when running a named profile.

## Supervision (systemd user unit)
openx installs a **systemd user service** by default on Linux/WSL2. We
...
`openx gateway install` writes the user unit. `openx doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/openx-gateway[-<profile>].service`:
```
[Unit]
Description=openx Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/openx gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now openx-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/openx-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now openx-gateway[-<profile>].service
```
...
## Operational checks
- Liveness: open WS and send `req:connect` â†?expect `res` with `payload.type="hello-ok"` (with snapshot).
- Readiness: call `health` â†?expect `ok: true` and a linked channel in `linkChannel` (when applicable).
...
## CLI helpers
- `openx gateway health|status` â€?request health/status over the Gateway WS.
- `openx message send --target <num> --message "hi" [--media ...]` â€?send via Gateway (idempotent for WhatsApp).
- `openx agent --message "hi" --to <num>` â€?run an agent turn (waits for final by default).
- `openx gateway call <method> --params '{"k":"v"}'` â€?raw method invoker for debugging.
- `openx gateway stop|restart` â€?stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `openx gateway` and the legacy TCP control port.
