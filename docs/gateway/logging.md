...
openx has two log â€œsurfacesâ€?
...
- Default rolling log file is under `/tmp/openx/` (one file per day): `openx-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.clawdbot/openx.json`:
  - `logging.file`
  - `logging.level`
...
`openx gateway` supports a per-gateway style switch:
...
openx gateway --verbose --ws-log compact

# show all WS traffic (full meta)
openx gateway --verbose --ws-log full
```
