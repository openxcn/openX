...
openx’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`openx agent`), so routing/permissions/config match your Gateway.
...
- `model: "openx:<agentId>"` (example: `"openx:main"`, `"openx:beta"`)
- `model: "agent:<agentId>"` (alias)

Or target a specific openx agent by header:

- `x-openx-agent-id: <agentId>` (default: `main`)

Advanced:
- `x-openx-session-key: <sessionKey>` to fully control session routing.
...
- The most recent `user` or `function_call_output` item becomes the “current message.�?- Earlier user/assistant messages are included as history for context.
