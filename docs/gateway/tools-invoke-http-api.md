...
openxâ€™s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled, but gated by Gateway auth and tool policy.
...
- `x-openx-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-openx-account-id: <accountId>` (when multiple accounts exist)
