---
summary: "Quick troubleshooting guide for common openx failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When openx misbehaves, here's how to fix it.
...
|---|---|---|
| `openx status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `openx status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `openx status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured�?doesn’t mean “working�?|
| `openx gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `openx channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `openx gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded�?but nothing runs |
| `openx logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `openx status --all` (it redacts tokens). If you paste `openx status`, consider setting `CLAWDBOT_SHOW_SECRETS=0` first (token previews).
...
**Logs:**
- Preferred: `openx logs --follow`
- File logs (always): `/tmp/openx/openx-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CLAWDBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u openx-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  openx config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  openx config set gateway.mode remote
  openx config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `openx setup` to create a starter config, then rerun
the gateway.
...
service was installed with Bun or a version-managed Node path, run `openx doctor`
...
- then run `openx sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `openx gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `openx gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `openx gateway status` and `openx doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
...
**If `openx gateway status` shows a config mismatch**
...
- Fix: rerun `openx gateway install --force` from the same `--profile` / `CLAWDBOT_STATE_DIR` you want the service to use.

**If `openx gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `openx doctor` to update it (or `openx gateway install --force` for a full rewrite).
...
**If `openx gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/openx` on disk.
...
openx intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `openx models list` or
  `openx models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdbot/openx.json}"
```
...
tail -f "$(ls -t /tmp/openx/openx-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
openx logs --follow | grep "pairing request"
```
...
openx logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/openx/openx-*.log | head -1)" | tail -20
```

### High Memory Usage

openx keeps conversation history in memory.
...
openx now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `openx doctor` reports every invalid entry.
- `openx doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `openx logs`, `openx health`, `openx status`, `openx gateway status`, and `openx gateway probe` still run even if the config is invalid.

### “All models failed�?�?what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/openx/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `openx models status` (CLI).
...
5) Run `openx channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. openx scrubs/normalizes tool
...
```bash
tccutil reset All bot.openx.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/openx/openx/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
openx gateway status
openx gateway stop
# Or: launchctl bootout gui/$UID/bot.openx.gateway (replace with bot.openx.<profile>; legacy com.clawdbot.* still works)
```
...
Ensure the global `openx` CLI is installed and matches the app version:
```bash
openx --version
npm install -g openx@<version>
```
...
#   ${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdbot/openx.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
openx gateway --verbose
openx channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/openx/openx-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CLAWDBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdbot/logs/...`; profiles use `~/.clawdbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u openx-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "openx Gateway (<profile>)" /V /FO LIST` |
...
openx channels login         # re-pair WhatsApp
openx gateway restart           # or: openx gateway
```
...
1. Check logs first: `/tmp/openx/` (default: `openx-YYYY-MM-DD.log`, or your configured `logging.file`)
