---
summary: "Frequently asked questions about openx setup, configuration, and usage"
---
# FAQ
...
- [Quick start and first-run setup](#quick-start-and-firstrun-setup)
  ...
  - [What’s the recommended way to install and set up openx?](#whats-the-recommended-way-to-install-and-set-up-openx)
  ...
  - [I can't access docs.openx.bot (SSL error). What now?](#i-cant-access-docsopenx-ssl-error-what-now)
  - [What’s the difference between stable and beta?](#whats-the-difference-between-stable-and-beta)
- [How do I install the beta version, and what’s the difference between beta and dev?](#how-do-i-install-the-beta-version-and-whats-the-difference-between-beta-and-dev)
  - [How do I try the latest bits?](#how-do-i-try-the-latest-bits)
  - [How long does install and onboarding usually take?](#how-long-does-install-and-onboarding-usually-take)
  - [Installer stuck? How do I get more feedback?](#installer-stuck-how-do-i-get-more-feedback)
  - [Windows install says git not found or openx not recognized](#windows-install-says-git-not-found-or-openx-not-recognized)
  - [The docs didn’t answer my question - how do I get a better answer?](#the-docs-didnt-answer-my-question-how-do-i-get-a-better-answer)
  - [How do I install openx on Linux?](#how-do-i-install-openx-on-linux)
  - [How do I install openx on a VPS?](#how-do-i-install-openx-on-a-vps)
  ...
  - [If I buy a Mac mini to run openx, can I connect it to my MacBook Pro?](#if-i-buy-a-mac-mini-to-run-openx-can-i-connect-it-to-my-macbook-pro)
  - [Can I use Bun?](#can-i-use-bun)
  - [Telegram: what goes in `allowFrom`?](#telegram-what-goes-in-allowfrom)
  - [Can multiple people use one WhatsApp number with different openxs?](#can-multiple-people-use-one-whatsapp-number-with-different-openxs)
  - [Can I run a "fast chat" agent and an "Opus for coding" agent?](#can-i-run-a-fast-chat-agent-and-an-opus-for-coding-agent)
  - [Does Homebrew work on Linux?](#does-homebrew-work-on-linux)
  - [What’s the difference between the hackable (git) install and npm install?](#whats-the-difference-between-the-hackable-git-install-and-npm-install)
  - [Can I switch between npm and git installs later?](#can-i-switch-between-npm-and-git-installs-later)
  - [Should I run the Gateway on my laptop or a VPS?](#should-i-run-the-gateway-on-my-laptop-or-a-vps)
  - [How important is it to run openx on a dedicated machine?](#how-important-is-it-to-run-openx-on-a-dedicated-machine)
  - [What are the minimum VPS requirements and recommended OS?](#what-are-the-minimum-vps-requirements-and-recommended-os)
  - [Can I run openx in a VM and what are the requirements](#can-i-run-openx-in-a-vm-and-what-are-the-requirements)
- [What is openx?](#what-is-openx)
  - [What is openx, in one paragraph?](#what-is-openx-in-one-paragraph)
  - [What’s the value proposition?](#whats-the-value-proposition)
  - [I just set it up what should I do first](#i-just-set-it-up-what-should-i-do-first)
  - [What are the top five everyday use cases for openx](#what-are-the-top-five-everyday-use-cases-for-openx)
  - [Can openx help with lead gen outreach ads and blogs for a SaaS](#can-openx-help-with-lead-gen-outreach-ads-and-blogs-for-a-saas)
...
- [Skills and automation](#skills-and-automation)
  ...
  - [Can openx run tasks on a schedule or continuously in the background?](#can-openx-run-tasks-on-a-schedule-or-continuously-in-the-background)
...
- [Where things live on disk](#where-things-live-on-disk)
  - [Is all data used with openx saved locally?](#is-all-data-used-with-openx-saved-locally)
  - [Where does openx store its data?](#where-does-openx-store-its-data)
  - [Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?](#where-should-agentsmd-soulmd-usermd-memorymd-live)
  - [What’s the recommended backup strategy?](#whats-the-recommended-backup-strategy)
  - [How do I completely uninstall openx?](#how-do-i-completely-uninstall-openx)
...
- [Config basics](#config-basics)
  ...
  - [Can the openx browser run headless?](#can-the-openx-browser-run-headless)
...
- [Remote gateways + nodes](#remote-gateways-nodes)
  ...
  - [Can two openxs talk to each other (local + VPS)?](#can-two-openxs-talk-to-each-other-local-vps)
...
- [Env vars and .env loading](#env-vars-and-env-loading)
  - [How does openx load environment variables?](#how-does-openx-load-environment-variables)
...
- [Sessions & multiple chats](#sessions-multiple-chats)
  ...
  - [Is there a way to make a team of openxs one CEO and many agents](#is-there-a-way-to-make-a-team-of-openxs-one-ceo-and-many-agents)
  - [Why did context get truncated mid-task? How do I prevent it?](#why-did-context-get-truncated-midtask-how-do-i-prevent-it)
  - [How do I completely reset openx but keep it installed?](#how-do-i-completely-reset-openx-but-keep-it-installed)
  ...
  - [Why doesn’t openx reply in a group?](#why-doesnt-openx-reply-in-a-group)
...
- [Gateway: ports, “already running�? and remote mode](#gateway-ports-already-running-and-remote-mode)
  ...
  - [Why does `openx gateway status` say `Runtime: running` but `RPC probe: failed`?](#why-does-openx-gateway-status-say-runtime-running-but-rpc-probe-failed)
  - [Why does `openx gateway status` show `Config (cli)` and `Config (service)` different?](#why-does-openx-gateway-status-show-config-cli-and-config-service-different)
  - [What does “another gateway instance is already listening�?mean?](#what-does-another-gateway-instance-is-already-listening-mean)
  - [How do I run openx in remote mode (client connects to a Gateway elsewhere)?](#how-do-i-run-openx-in-remote-mode-client-connects-to-a-gateway-elsewhere)
...
- [Logging and debugging](#logging-and-debugging)
  ...
  - [I closed my terminal on Windows - how do I restart openx?](#i-closed-my-terminal-on-windows-how-do-i-restart-openx)
  ...
  - [ELI5: `openx gateway restart` vs `openx gateway`](#eli5-openx-gateway-restart-vs-openx-gateway)
  - [What’s the fastest way to get more details when something fails?](#whats-the-fastest-way-to-get-more-details-when-something-fails)
- [Media & attachments](#media-attachments)
  - [My skill generated an image/PDF, but nothing was sent](#my-skill-generated-an-imagepdf-but-nothing-was-sent)
- [Security and access control](#security-and-access-control)
  - [Is it safe to expose openx to inbound DMs?](#is-it-safe-to-expose-openx-to-inbound-dms)
...
5) **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/openx/openx-*.log | head -1)"
   ```
...
7) **Gateway snapshot**
   ```bash
   openx health --json
   openx health --verbose   # shows the target URL + config path on errors
   ```
...
```bash
curl -fsSL https://openx.bot/install.sh | bash -s -- --install-method git
```

This installs openx **from a git checkout**, so the agent can read the code + docs and
...
https://github.com/openx/openx/issues
https://github.com/openx/openx/pulls
...
What they do:
- `openx status`: quick snapshot of gateway/agent health + basic config.
- `openx models status`: checks provider auth + model availability.
- `openx doctor`: validates and repairs common config/state issues.

Other useful CLI checks: `openx status --all`, `openx logs --follow`,
`openx gateway status`, `openx health --verbose`.

Quick debug loop: [First 60 seconds if something's broken](#first-60-seconds-if-somethings-broken).
Install docs: [Install](/install), [Installer flags](/install/installer), [Updating](/install/updating).

### Whats the recommended way to install and set up openx
...
If you don’t have a global install yet, run it via `pnpm openx onboard`.
...
- If it asks for auth, run `openx dashboard` and use the tokenized link (`?token=...`).
- The token is the same value as `gateway.auth.token` (or `CLAWDBOT_GATEWAY_TOKEN`) and is stored by the UI after first load.

**Not on localhost:**
- **Tailscale Serve** (recommended): keep bind loopback, run `openx gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy auth (no token).
- **Tailnet bind**: run `openx gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
- **SSH tunnel**: `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/?token=...` from `openx dashboard`.
...
Related: [Migrating](/install/migrating), [Where things live on disk](/help/faq#where-does-openx-store-its-data),
...
https://github.com/openx/openx/blob/main/CHANGELOG.md

Newest entries are at the top. If the top section is marked **Unreleased**, the next dated
section is the latest shipped version. Entries are grouped by **Highlights**, **Changes**, and
**Fixes** (plus docs/other sections when needed).

### I cant access docsopenx SSL error What now

Some Comcast/Xfinity connections incorrectly block `docs.openx.bot` via Xfinity
Advanced Security. Disable it or allowlist `docs.openx.bot`, then retry. More
detail: [Troubleshooting](/help/troubleshooting#docsopenx-shows-an-ssl-error-comcastxfinity).
Please help us unblock it by reporting here: https://spa.xfinity.com/check_url_status.

If you still can't reach the site, the docs are mirrored on GitHub:
https://github.com/openx/openx/tree/main/docs
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openx.bot/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://openx.bot/install.sh | bash -s -- --install-method git
```

Windows installer (PowerShell):
https://openx.bot/install.ps1
...
```bash
curl -fsSL https://openx.bot/install.sh | bash -s -- --install-method git --verbose
```

More options: [Installer flags](/install/installer).

### Windows install says git not found or openx not recognized
...
**2) openx is not recognized after install**
...
### How do I install openx on Linux
...
### How do I install openx on a VPS
...
openx update
openx update status
openx update --channel stable|beta|dev
openx update --tag <dist-tag|version>
openx update --no-restart
```

If you must automate from an agent:

```bash
openx update --yes --no-restart
openx gateway restart
```
...
`openx onboard` is the recommended setup path. In **local mode** it walks you through:
...
No. You can run openx with **API keys** (Anthropic/OpenAI/others) or with
...
`claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in the wizard or paste it with `openx models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
...
Copy the token it prints, then choose **Anthropic token (paste setup-token)** in the wizard. If you want to run it on the gateway host, use `openx models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `openx models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).

### Do you support Claude subscription auth (Claude Pro/Max)

Yes �?via **setup-token**. openx no longer reuses Claude Code CLI OAuth tokens; use a setup-token or an Anthropic API key. Generate the token anywhere and paste it on the gateway host. See [Anthropic](/providers/anthropic) and [OAuth](/concepts/oauth).
...
Tip: set a **fallback model** so openx can keep replying while a provider is rate‑limited.
...
openx supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). The wizard can run the OAuth flow and will set the default model to `openai-codex/gpt-5.2` when appropriate. See [Model providers](/concepts/model-providers) and [Wizard](/start/wizard).

### Do you support OpenAI subscription auth Codex OAuth

Yes. openx fully supports **OpenAI Code (Codex) subscription OAuth**. The onboarding wizard
...
Gemini CLI uses a **plugin auth flow**, not a client id or secret in `openx.json`.

Steps:
1) Enable the plugin: `openx plugins enable google-gemini-cli-auth`
2) Login: `openx models auth login --provider google-gemini-cli --set-default`

This stores OAuth tokens in auth profiles on the gateway host. Details: [Model providers](/concepts/model-providers).

### Is a local model OK for casual chats

Usually no. openx needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.1 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
...
No. openx runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
...
any Mac works. openx’s iMessage integrations run on macOS (BlueBubbles or `imsg`), while
...
### If I buy a Mac mini to run openx can I connect it to my MacBook Pro
...
- Use `openx nodes status` / `openx nodes list` to see it.
...
- DM your bot, then run `openx logs --follow` and read `from.id`.
...
### Can multiple people use one WhatsApp number with different openxs
...
If you run openx via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non‑login shells.
...
This **does not delete your data** - it only changes the openx code install. Your state
...
**openx-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

**Recommended default:** VPS if you had gateway disconnects before. Local is great when you’re actively using the Mac and want local file access or UI automation with a visible browser.

### How important is it to run openx on a dedicated machine
...
openx is lightweight. For a basic Gateway + one chat channel:
...
### Can I run openx in a VM and what are the requirements
...
### What is openx in one paragraph

openx is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.

### Whats the value proposition

openx is not “just a Claude wrapper.�?It’s a **local-first control plane** that lets you run a
...
### What are the top five everyday use cases for openx
...
### Can openx help with lead gen outreach ads and blogs for a SaaS
...
openx draft and you approve.

Docs: [Security](/gateway/security).

### What are the advantages vs Claude Code for web development

openx is a **personal assistant** and coordination layer, not an IDE replacement. Use
Claude Code or Codex for the fastest direct coding loop inside a repo. Use openx when you
...
Showcase: https://openx.bot/showcase

## Skills and automation

### How do I customize skills without keeping the repo dirty

Use managed overrides instead of editing the repo copy. Put your changes in `~/.clawdbot/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.clawdbot/openx.json`). Precedence is `<workspace>/skills` > `~/.clawdbot/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.

### Can I load skills from a custom folder

Yes. Add extra directories via `skills.load.extraDirs` in `~/.clawdbot/openx.json` (lowest precedence). Default precedence remains: `<workspace>/skills` �?`~/.clawdbot/skills` �?bundled �?`skills.load.extraDirs`. `clawdhub` installs into `./skills` by default, which openx treats as `<workspace>/skills`.
...
Debug:
```bash
openx cron run <jobId> --force
openx cron runs --id <jobId> --limit 50
```
...
### Can openx run tasks on a schedule or continuously in the background
...
Not directly. macOS skills are gated by `metadata.openx.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `imsg`, `apple-notes`, `apple-reminders`) will not load unless you override the gating.

You have three supported patterns:

**Option A - run the Gateway on a Mac (simplest).**  
Run the Gateway where the macOS binaries exist, then connect from Linux in [remote mode](#how-do-i-run-openx-in-remote-mode-client-connects-to-a-gateway-elsewhere) or over Tailscale. The skills load normally because the Gateway host is macOS.

**Option B - use a macOS node (no SSH).**  
Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. openx can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
...
3) Override the skill metadata (workspace or `~/.clawdbot/skills`) to allow Linux:
   ...
   metadata: {"openx":{"os":["darwin","linux"],"requires":{"bins":["imsg"]}}}
   ---
   ```
4) Start a new session so the skills snapshot refreshes.

For iMessage specifically, you can also point `channels.imessage.cliPath` at an SSH wrapper (openx only needs stdio). See [iMessage](/channels/imessage).
...
ClawdHub installs into `./skills` under your current directory (or falls back to your configured openx workspace); openx treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.clawdbot/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawdHub](/tools/clawdhub).
...
openx memory is just Markdown files in the agent workspace:
- Daily notes in `memory/YYYY-MM-DD.md`
- Curated long-term notes in `MEMORY.md` (main/private sessions only)

openx also runs a **silent pre-compaction memory flush** to remind the model
...
If you don’t set a provider explicitly, openx auto-selects a provider when it
can resolve an API key (auth profiles, `models.providers.*.apiKey`, or env vars).
It prefers OpenAI if an OpenAI key resolves, otherwise Gemini if a Gemini key
resolves. If neither key is available, memory search stays disabled until you
configure it. If you have a local model path configured and present, openx
prefers `local`.
...
### Is all data used with openx saved locally

No - **openx’s state is local**, but **external services still see what you send them**.
...
### Where does openx store its data

Everything lives under `$CLAWDBOT_STATE_DIR` (default: `~/.clawdbot`):

| Path | Purpose |
|------|---------|
| `$CLAWDBOT_STATE_DIR/openx.json` | Main config (JSON5) |
...
Legacy single‑agent path: `~/.clawdbot/agent/*` (migrated by `openx doctor`).
...
### How do I completely uninstall openx
...
`workspace` to the repo root. The openx repo is just source code; keep the
...
openx reads an optional **JSON5** config from `$CLAWDBOT_CONFIG_PATH` (default: `~/.clawdbot/openx.json`):
...
If you **really** want open loopback, remove `gateway.auth` from your config. Doctor can generate a token for you any time: `openx doctor --generate-gateway-token`.
...
key. **Recommended:** run `openx configure --section web` to store it in
...
### Can the openx browser run headless
...
Check the basics:
- Gateway is running: `openx gateway status`
- Gateway health: `openx status`
- Channel health: `openx channels status`
...
### Can two openxs talk to each other local VPS
...
`openx agent --message ... --deliver`, targeting a chat where the other bot
listens. If one bot is on a remote VPS, point your CLI at that remote Gateway
via SSH/Tailscale (see [Remote access](/gateway/remote)).

Example pattern (run from a machine that can reach the target Gateway):
```bash
openx agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
```
...
Recover:
- Restore from backup (git or a copied `~/.clawdbot/openx.json`).
- If you have no backup, re-run `openx doctor` and reconfigure channels/models.
- If this was unexpected, file a bug and include your last known config or any backup.
- A local coding agent can often reconstruct a working config from logs or history.

Avoid it:
- Use `openx config set` for small changes.
- Use `openx configure` for interactive edits.
...
### How does openx load environment variables

openx reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:
...
`openx models status` reports whether **shell env import** is enabled. “Shell env: off�?does **not** mean your env vars are missing - it just means openx won’t load
...
### Is there a way to make a team of openxs one CEO and many agents
...
### How do I completely reset openx but keep it installed
...
```bash
openx reset --scope full --yes --non-interactive
```
...
- Dev reset: `openx gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
...
headers like `# Heading`), openx skips the heartbeat run to save API calls.
...
No. openx runs on **your own account**, so if you’re in the group, openx can see it.
...
```bash
openx directory groups list --channel whatsapp
```

Docs: [WhatsApp](/channels/whatsapp), [Directory](/cli/directory), [Logs](/cli/logs).

### Why doesnt openx reply in a group
...
- Use `openx doctor` to spot stray workspaces and profile mismatches.
...
openx’s default model is whatever you set as:

```
agents.defaults.model.primary
```

Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-5`). If you omit the provider, openx currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
...
- `openx models set ...` (updates just model config)
- `openx configure --section models` (interactive)
- edit `agents.defaults.model` in `~/.clawdbot/openx.json`

Avoid `config.apply` with a partial object unless you intend to replace the whole config.
If you did overwrite config, restore from backup or re-run `openx doctor` to repair.
...
Yes. openx ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
...
Fix options:
- Run `openx agents add <id>` and configure auth during the wizard.
...
Cooldowns apply to failing profiles (exponential backoff), so openx can keep responding even when a provider is rate‑limited or temporarily failing.
...
- **Confirm where auth profiles live** (new vs legacy paths)
  ...
  - Legacy: `~/.clawdbot/agent/*` (migrated by `openx doctor`)
...
- **Sanity‑check model/auth status**
  - Use `openx models status` to see configured models and whether providers are authenticated.
...
- **Use a setup-token**
  - Run `claude setup-token`, then paste it with `openx models auth setup-token --provider anthropic`.
  - If the token was created on another machine, use `openx models auth paste-token --provider anthropic`.
...
If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), openx will try it during model fallback. If you haven’t configured Google credentials, you’ll see `No API key found for provider "google"`.
...
Fix: openx now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
openx uses provider‑prefixed IDs like:
...
openx may temporarily skip a profile if it’s in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `openx models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.
...
openx models auth order set --provider anthropic anthropic:work anthropic:default
...
```bash
openx models auth order set --provider anthropic --agent main anthropic:default
```
...
### Why does openx gateway status say Runtime running but RPC probe failed

Because “running�?is the **supervisor’s** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

Use `openx gateway status` and trust these lines:
- `Probe target:` (the URL the probe actually used)
- `Listening:` (what’s actually bound on the port)
- `Last gateway error:` (common root cause when the process is alive but the port isn’t listening)

### Why does openx gateway status show Config cli and Config service different

You’re editing one config file while the service is running another (often a `--profile` / `CLAWDBOT_STATE_DIR` mismatch).

Fix:
```bash
openx gateway install --force
```
Run that from the same `--profile` / environment you want the service to use.

### What does another gateway instance is already listening mean

openx enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

Fix: stop the other instance, free the port, or run with `openx gateway --port <port>`.

### How do I run openx in remote mode client connects to a Gateway elsewhere
...
Notes:
- `openx gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
...
Facts (from code):
- The Control UI stores the token in browser localStorage key `openx.control.settings.v1`.
- The UI can import `?token=...` (and/or `?password=...`) once, then strips it from the URL.

Fix:
- Fastest: `openx dashboard` (prints + copies tokenized link, tries to open; shows SSH hint if headless).
- If you don’t have a token yet: `openx doctor --generate-gateway-token`.
...
- Still stuck? Run `openx status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
...
- Use `openx --profile <name> …` per instance (auto-creates `~/.clawdbot-<name>`).
- Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
- Install a per-profile service: `openx --profile <name> gateway install`.

Profiles also suffix service names (`bot.openx.<profile>`; legacy `com.clawdbot.*`, `openx-gateway-<profile>.service`, `openx Gateway (<profile>)`).
...
```
openx tui --url ws://<host>:18789 --token <token>
```
...
```
/tmp/openx/openx-YYYY-MM-DD.log
```
...
- Linux: `journalctl --user -u openx-gateway[-<profile>].service -n 200 --no-pager`
- Windows: `schtasks /Query /TN "openx Gateway (<profile>)" /V /FO LIST`
...
If you run the gateway manually, `openx gateway --force` can reclaim the port. See [Gateway](/gateway).

### I closed my terminal on Windows how do I restart openx
...
1) Is the Gateway running? `openx gateway status`
2) Is the Gateway healthy? `openx status`
3) Does the UI have the right token? `openx dashboard`
...
```bash
openx channels status
openx channels logs --channel telegram
```
...
### ELI5 openx gateway restart vs openx gateway

- `openx gateway restart`: restarts the **background service** (launchd/systemd).
- `openx gateway`: runs the gateway **in the foreground** for this terminal session.

If you installed the service, use the gateway commands. Use `openx gateway` when
...
Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [openx assistant setup](/start/clawd) and [Agent send](/tools/agent-send).

CLI sending:

```bash
openx message send --target +15555550123 --message "Here you go" --media /path/to/file.png
```
...
### Is it safe to expose openx to inbound DMs

Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

- Default behavior on DM‑capable channels is **pairing**:
  - Unknown senders receive a pairing code; the bot does not process their message.
  - Approve with: `openx pairing approve <channel> <code>`
  - Pending requests are capped at **3 per channel**; check `openx pairing list <channel>` if a code didn’t arrive.
- Opening DMs publicly requires explicit opt‑in (`dmPolicy: "open"` and allowlist `"*"`).

Run `openx doctor` to surface risky DM policies.
...
No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. openx only replies to chats it receives or to explicit sends you trigger.

Approve pairing with:

```bash
openx pairing approve whatsapp <code>
```

List pending requests:

```bash
openx pairing list whatsapp
```
...
openx blocks **cross‑provider** messaging by default. If a tool call is bound
...
**A:** In openx, credentials and model selection are separate. Setting `ANTHROPIC_API_KEY` (or storing an Anthropic API key in auth profiles) enables authentication, but the actual default model is whatever you configure in `agents.defaults.model.primary` (for example, `anthropic/claude-sonnet-4-5` or `anthropic/claude-opus-4-5`). If you see `No credentials found for profile "anthropic:default"`, it means the Gateway couldn’t find Anthropic credentials in the expected `auth-profiles.json` for the agent that’s running.

---

Still stuck? Ask in [Discord](https://discord.com/invite/clawd) or open a [GitHub discussion](https://github.com/openx/openx/discussions).
