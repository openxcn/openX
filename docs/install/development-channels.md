...
openx ships three update channels:
...
When you **explicitly** switch channels with `--channel`, openx also aligns
the install method:

- `dev` ensures a git checkout (default `~/openx`, override with `CLAWDBOT_GIT_DIR`),
...
When you switch channels with `openx update`, openx also syncs plugin sources:
