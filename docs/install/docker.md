---
summary: "Optional Docker-based setup and onboarding for openx"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run openx on a host without local installs.
...
This guide covers:
- Containerized Gateway (full openx in Docker)
...
```bash
docker build -t openx:local -f Dockerfile .
docker compose run --rm openx-cli onboard
...
`openx-gateway` and `openx-cli` by generating `docker-compose.extra.yml`.
...
```bash
export CLAWDBOT_HOME_VOLUME="openx_home"
./docker-setup.sh
```
...
WhatsApp (QR):
```bash
docker compose run --rm openx-cli channels login
```

Telegram (bot token):
```bash
docker compose run --rm openx-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):
```bash
docker compose run --rm openx-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec openx-gateway node dist/index.js health --token "$CLAWDBOT_GATEWAY_TOKEN"
```
...
- Image: `openx-sandbox:bookworm-slim`
...
openx auto-recreates containers when `setupCommand` (or docker config) changes
unless the container was **recently used** (within ~5 minutes). Hot containers
log a warning with the exact `openx sandbox recreate ...` command.
...
This builds `openx-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `openx-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: { defaults: { sandbox: { docker: { image: "openx-sandbox-common:bookworm-slim" } } } }
}
```
...
This builds `openx-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-openx-browser" } }
    }
  }
}
```
...
```bash
docker build -t my-openx-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-openx-sbx" } }
    }
  }
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/openx/openx/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: openx runs commands with `sh -lc` (login shell), which
