---
summary: "Move (migrate) a openx install from one machine to another"
read_when:
  - You are moving openx to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
---
# Migrating openx to a new machine

This guide migrates a openx Gateway from one machine to another **without redoing onboarding**.

The migration is simple conceptually:

- Copy the **state directory** (`$CLAWDBOT_STATE_DIR`, default: `~/.clawdbot/`) â€?this includes config, auth, sessions, and channel state.
- Copy your **workspace** (`~/clawd/` by default) â€?this includes your agent files (memory, prompts, etc.).
...
- Gateway configuration (`openx.json`)
...
cd ~
tar -czf openx-state.tgz .clawdbot
...
### Step 1 â€?Install openx on the new machine

On the **new** machine, install the CLI (and Node if needed):

- See: [Install](/install)

At this stage, itâ€™s OK if onboarding creates a fresh `~/.clawdbot/` â€?you will overwrite it in the next step.
...
If you ran the old gateway with a profile (or `CLAWDBOT_STATE_DIR`), and the new gateway uses a different one, youâ€™ll see symptoms like:
...
### Footgun: copying only `openx.json`

`openx.json` is not enough. Many providers store state under:
...
- `openx status` shows the gateway running
...
- [Where does openx store its data?](/help/faq#where-does-openx-store-its-data)
