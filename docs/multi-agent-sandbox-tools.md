...
For debugging â€œwhy is this blocked?â€? see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `openx sandbox explain`.
...
- `group:nodes`: `nodes`
- `group:openx`: all built-in openx tools (excludes provider plugins)
...
Legacy `agent.*` configs are migrated by `openx doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
1. **Check agent resolution:**
   ```exec
   openx agents list --bindings
   ```

2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=openx.sandbox=1"
   ```
