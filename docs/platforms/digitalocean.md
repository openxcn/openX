---
summary: "openx on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up openx on DigitalOcean
  - Looking for cheap VPS hosting for openx
---

# openx on DigitalOcean

## Goal

Run a persistent openx Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
| Hetzner | CX22 | 2 vCPU, 4GB RAM | â‚?.79 (~$4) | Cheapest paid option |
...
# Check service
systemctl --user status openx-gateway.service

# View logs
journalctl --user -u openx-gateway.service -f
```
...
openx pairing approve telegram <CODE>
```

### WhatsApp
```bash
openx channels login whatsapp
# Scan QR code
```
...
- `~/clawd/` â€?workspace (SOUL.md, memory, etc.)
...
journalctl -u openx --no-pager -n 50
```
