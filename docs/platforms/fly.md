---
title: Fly.io
description: Deploy openx on Fly.io
---

# Fly.io Deployment

**Goal:** openx Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-openx

# Create a persistent volume (1GB is usually enough)
fly volumes create openx_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `openx.json` where they could be accidentally exposed or logged.
...
**Note:** With `CLAWDBOT_STATE_DIR=/data`, the config path is `/data/openx.json`.
...
Or visit `https://my-openx.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/openx.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/openx.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/openx.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/openx.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/openx.json"
```
...
```bash
# List current IPs
fly ips list -a my-openx

# Release public IPs
fly ips release <public-ipv4> -a my-openx
fly ips release <public-ipv6> -a my-openx
...
fly ips allocate-v6 --private -a my-openx
```
...
fly proxy 3000:3000 -a my-openx
