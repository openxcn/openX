...
service via the external `openx` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.openx.gateway`
  (or `bot.openx.<profile>` when using `--profile`/`CLAWDBOT_PROFILE`; legacy `com.clawdbot.*` is supported).
