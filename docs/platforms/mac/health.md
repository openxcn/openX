...
## How the probe works
- App runs `openx health --json` via `ShellExecutor` every ~60s and on demand. The probe loads creds and reports status without sending messages.
- Cache the last good snapshot and the last error separately to avoid flicker; show the timestamp of each.

## When in doubt
- You can still use the CLI flow in [Gateway health](/gateway/health) (`openx status`, `openx status --deep`, `openx health --json`) and tail `/tmp/openx/openx-*.log` for `web-heartbeat` / `web-reconnect`.
