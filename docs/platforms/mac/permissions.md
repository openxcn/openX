...
- Same path: run the app from a fixed location (for openx, `dist/openx.app`).
...
```bash
sudo tccutil reset Accessibility bot.openx.mac
sudo tccutil reset ScreenCapture bot.openx.mac
