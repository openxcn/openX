---
summary: "openx on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up openx on a Raspberry Pi
  - Running openx on ARM devices
  - Building a cheap always-on personal AI
---

# openx on Raspberry Pi

## Goal

Run a persistent, always-on openx Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
|----------|-----|--------|-------|
| **Pi 5** | 4GB/8GB | ‚ú?Best | Fastest, recommended |
| **Pi 4** | 4GB | ‚ú?Good | Sweet spot for most users |
| **Pi 4** | 2GB | ‚ú?OK | Works, add swap |
| **Pi 4** | 1GB | ‚ö†Ô∏è Tight | Possible with swap, minimal config |
| **Pi 3B+** | 1GB | ‚ö†Ô∏è Slow | Works but sluggish |
| **Pi Zero 2 W** | 512MB | ‚ù?| Not recommended |
...
Use **Raspberry Pi OS Lite (64-bit)** ‚Ä?no desktop needed for a headless server.
...
The hackable install gives you direct access to logs and code ‚Ä?useful for debugging ARM-specific issues.
...
# Update config
openx config set gateway.bind tailnet
...
Most openx features work on ARM64, but some external binaries may need ARM builds:

| Tool | ARM64 Status | Notes |
|------|--------------|-------|
| Node.js | ‚ú?| Works great |
| WhatsApp (Baileys) | ‚ú?| Pure JS, no issues |
| Telegram | ‚ú?| Pure JS, no issues |
| gog (Gmail CLI) | ‚ö†Ô∏è | Check for ARM release |
| Chromium (browser) | ‚ú?| `sudo apt install chromium-browser` |
...
**Don't try to run local LLMs on a Pi** ‚Ä?even small models are too slow. Let Claude/GPT do the heavy lifting.
...
sudo systemctl is-enabled openx

# Enable if not
sudo systemctl enable openx

# Start on boot
sudo systemctl start openx
```
...
```bash
# Check logs
journalctl -u openx --no-pager -n 100

# Common fix: rebuild
cd ~/openx  # if using hackable install
npm run build
sudo systemctl restart openx
```
...
- [Linux guide](/platforms/linux) ‚Ä?general Linux setup
- [DigitalOcean guide](/platforms/digitalocean) ‚Ä?cloud alternative
- [Hetzner guide](/platforms/hetzner) ‚Ä?Docker setup
- [Tailscale](/gateway/tailscale) ‚Ä?remote access
- [Nodes](/nodes) ‚Ä?pair your laptop/phone with the Pi gateway
