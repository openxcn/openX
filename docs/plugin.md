---
summary: "openx plugins/extensions: discovery, config, and safety"
read_when:
...
A plugin is just a **small code module** that extends openx with extra
features (commands, tools, and Gateway RPC).

Most of the time, youâ€™ll use plugins when you want a feature thatâ€™s not built
into core openx yet (or you want to keep optional features out of your main
install).
...
```bash
openx plugins install @openx/voice-call
```
...
- Microsoft Teams is plugin-only as of 2026.1.15; install `@openx/msteams` if you use Teams.
...
- [Voice Call](/plugins/voice-call) â€?`@openx/voice-call`
- [Zalo Personal](/plugins/zalouser) â€?`@openx/zalouser`
- [Matrix](/channels/matrix) â€?`@openx/matrix`
- [Nostr](/channels/nostr) â€?`@openx/nostr`
- [Zalo](/channels/zalo) â€?`@openx/zalo`
- [Microsoft Teams](/channels/msteams) â€?`@openx/msteams`
...
openx plugins are **TypeScript modules** loaded at runtime via jiti. **Config
...
4) Bundled extensions (shipped with openx, **disabled by default**)
- `<openx>/extensions/*`

Bundled plugins must be enabled explicitly via `plugins.entries.<id>.enabled`
or `openx plugins enable <id>`. Installed plugins are enabled by default,
but can be disabled the same way.

Each plugin must include a `openx.plugin.json` file in its root. If a path
...
A plugin directory may include a `package.json` with `openx.extensions`:
...
Channel plugins can advertise onboarding metadata via `openx.channel` and
install hints via `openx.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@openx/nextcloud-talk",
  "openx": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
openx can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "openx": { "channel": {...}, "install": {...} } } ] }`.
...
If a plugin exports `id`, openx uses it but warns when it doesnâ€™t match the
configured id.
...
- Plugin config is validated using the JSON Schema embedded in
  `openx.plugin.json` (`configSchema`).
...
openx augments `uiHints` at runtime based on discovered plugins:
...
openx plugins install <path>                 # copy a local file/dir into ~/.clawdbot/extensions/<id>
openx plugins install ./extensions/voice-call # relative path ok
openx plugins install ./plugin.tgz           # install from a local tarball
openx plugins install ./plugin.zip           # install from a local zip
openx plugins install -l ./extensions/voice-call # link (no copy) for dev
openx plugins install @openx/voice-call # install from npm
...
Plugins may also register their own topâ€‘level commands (example: `openx voicecall`).
...
```
import { registerPluginHooksFromDir } from "openx/plugin-sdk";
...
- Plugin-managed hooks show up in `openx hooks list` with `plugin:<id>`.
- You cannot enable/disable plugin-managed hooks via `openx hooks`; enable/disable the plugin instead.

## Provider plugins (model auth)

Plugins can register **model provider auth** flows so users can run OAuth or
API-key setup inside openx (no external scripts needed).

Register a provider via `api.registerProvider(...)`. Each provider exposes one
or more auth methods (OAuth, API key, device code, etc.). These methods power:

- `openx models auth login --provider <id> [--method <id>]`
...
- `config`: The current openx config
...
- Main package: `openx` (this repo)
- Plugins: separate npm packages under `@openx/*` (example: `@openx/voice-call`)

Publishing contract:

- Plugin `package.json` must include `openx.extensions` with one or more entry files.
- Entry files can be `.js` or `.ts` (jiti loads TS at runtime).
- `openx plugins install <npm-spec>` uses `npm pack`, extracts into `~/.clawdbot/extensions/<id>/`, and enables it in config.
...
- CLI: `openx voicecall start|status`
- Tool: `voice_call`
...
- Separately published plugins should run their own CI (lint/build/test) and validate `openx.extensions` points at the built entrypoint (`dist/index.js`).
