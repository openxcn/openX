...
read_when:
  - You are building a openx plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (openx.plugin.json)

Every plugin **must** ship a `openx.plugin.json` file in the **plugin root**.
openx uses this manifest to validate configuration **without executing plugin
