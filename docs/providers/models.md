---
summary: "Model providers (LLMs) supported by openx"
read_when:
...
openx can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `openx onboard`).
