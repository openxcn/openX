---
summary: "Use Synthetic's Anthropic-compatible API in openx"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. openx registers it as the
...
```bash
openx onboard --auth-choice synthetic-api-key
```
...
Note: openx's Anthropic client appends `/v1` to the base URL, so use
