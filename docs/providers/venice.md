---
summary: "Use Venice AI privacy-focused models in openx"
read_when:
  - You want privacy-focused inference in openx
...
```bash
openx onboard --auth-choice venice-api-key
```
...
```bash
openx chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, openx shows all available Venice models. Pick based on your needs:
...
```bash
openx models set venice/claude-opus-45
openx models set venice/llama-3.3-70b
```

List all available models:

```bash
openx models list | grep venice
```

## Configure via `openx configure`

1. Run `openx configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
openx automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
openx chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
openx chat --model venice/claude-opus-45

# Use uncensored model
openx chat --model venice/venice-uncensored

# Use vision model with image
openx chat --model venice/qwen3-vl-235b-a22b

# Use coding model
openx chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `openx models list` to see currently available models. Some models may be temporarily offline.
