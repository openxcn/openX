...
Accessed via `openxPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: openxConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: openxConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: openxConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: openxConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: openxConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: openxConfig): string;
  };
};
```
...
- Add `api.runtime` to `openxPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `openxRuntime: ">=2026.2.0"`).
