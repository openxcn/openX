...
openx features that can generate provider usage or paid API calls.
...
- `openx status --usage` and `openx channels list` show provider **usage windows**
...
openx can pick up credentials from:
...
### 8) Model scan / probe
`openx models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
