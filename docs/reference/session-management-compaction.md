...
This document explains how openx manages sessions end-to-end:
...
openx is designed around a single **Gateway process** that owns session state.
...
openx persists sessions in two layers:
...
openx resolves these via `src/config/sessions.ts`.
...
openx intentionally does **not** “fix up�?transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (openx consumes the events, but Pi decides when to compact).
...
openx also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, openx bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, openx leaves it alone.
...
openx supports “silent�?turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user�?
- openx strips/suppresses this in the delivery layer.

As of `2026.1.10`, openx also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
openx uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but openx’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `openx status`.
