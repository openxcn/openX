...
Deploy openx on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/openx/openx" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run openx:
...
The web dashboard is available at `https://<your-service>.onrender.com/openx`.
...
If you use the original openx repository, Render will not auto-deploy your openx. To update it, run a manual Blueprint sync from the dashboard.
...
For openx, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any openx host.
