...
read_when:
  ...
  - You want the fastest path from install ‚Ü?onboarding ‚Ü?first message
---

# Getting Started

Goal: go from **zero** ‚Ü?**first working chat** (with sane defaults) as quickly as possible.

Fastest chat: open the Control UI (no channel setup needed). Run `openx dashboard`
and chat in the browser, or open `http://127.0.0.1:18789/` on the gateway host.
Docs: [Dashboard](/web/dashboard) and [Control UI](/web/control-ui).

Recommended path: use the **CLI onboarding wizard** (`openx onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `openx configure --section web` (stores `tools.web.search.apiKey`).
...
‚ö†Ô∏è **Bun warning (WhatsApp + Telegram):** Bun has known issues with these
...
Scan via WhatsApp ‚Ü?Settings ‚Ü?Linked Devices.
...
If you‚Äôre hacking on openx itself, run from source:
...
If you don‚Äôt have a global install yet, run the onboarding step via `pnpm openx ...` from the repo.
`pnpm build` also bundles A2UI assets; if you need to run just that step, use `pnpm canvas:a2ui:bundle`.

Gateway (from this repo):

```bash
node openx.mjs gateway --port 18789 --verbose
```
...
If `openx health` shows ‚Äúno auth configured‚Ä? go back to the wizard and set OAuth/key auth ‚Ä?the agent won‚Äôt be able to respond without it.

Tip: `openx status --all` is the best pasteable, read-only debug report.
Health probes: `openx health` (or `openx status --deep`) asks the running gateway for a health snapshot.
