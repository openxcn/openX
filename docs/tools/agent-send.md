---
summary: "Direct `openx agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `openx agent` (direct agent runs)

`openx agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `openx message --target`).
...
```bash
openx agent --to +15555550123 --message "status update"
...
openx agent --to +15555550123 --message "Summon reply" --deliver
