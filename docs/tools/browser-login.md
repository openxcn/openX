...
openx controls a **dedicated Chrome profile** (named `clawd`, orange‑tinted UI). This is separate from your daily browser profile.
...
```bash
openx browser start
openx browser open https://x.com
```
...
```bash
openx browser open https://x.com --browser-profile clawd --target host
```
