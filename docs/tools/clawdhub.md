...
4) Start a new openx session so it picks up the new skill.
...
## How it fits into openx

By default, the CLI installs skills into `./skills` under your current working directory. If a openx workspace is configured, `clawdhub` falls back to that workspace unless you override `--workdir` (or `CLAWDHUB_WORKDIR`). openx loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.clawdbot/skills` or bundled skills, workspace skills take precedence.
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to openx workspace).
...
`clawdhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/openx/skills` and `~/.clawdbot/skills`). This is designed to find older skill installs without extra flags.
