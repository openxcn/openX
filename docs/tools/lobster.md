---
title: Lobster
summary: "Typed workflow runtime for openx with resumable approval gates."
description: Typed workflow runtime for openx â€?composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets openx run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: openx runs one Lobster tool call and gets a structured result.
...
- **Approve/resume is built in**: A normal program can prompt a human, but it canâ€™t *pause and resume* with a durable token without you inventing that runtime yourself.
...
openx launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In openx tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the openx Gateway (see the [Lobster repo](https://github.com/openx/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), openx keeps core tools enabled. To restrict core
...
â†?clawd calls gmail.list
â†?LLM summarizes
â†?User: "draft replies to #2 and #5"
â†?LLM drafts
â†?User: "send #2"
â†?clawd calls gmail.send
...
- **No secrets** â€?Lobster doesn't manage OAuth; it calls clawd tools that do.
