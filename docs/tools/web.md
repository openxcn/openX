...
openx ships two lightweight web tools:
...
3) Run `openx configure --section web` to store the key in config (recommended), or set `BRAVE_API_KEY` in your environment.
...
**Recommended:** run `openx configure --section web`. It stores the key in
`~/.clawdbot/openx.json` under `tools.web.search.apiKey`.

**Environment alternative:** set `BRAVE_API_KEY` in the Gateway process
environment. For a gateway install, put it in `~/.clawdbot/.env` (or your
service environment). See [Env vars](/help/faq#how-does-openx-load-environment-variables).
...
If no base URL is set, openx chooses a default based on the API key source:
...
- `query` (required)
- `count` (1â€?0; default from config)
