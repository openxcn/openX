...
openx can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere openx can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, openx defaults
...
TTS config lives under `messages.tts` in `openx.json`.
...
- If `provider` is **unset**, openx prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
- **Edge TTS**: uses `edge.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  - `node-edge-tts` accepts an `outputFormat`, but not all formats are available
    from the Edge service. citeturn2search0�?  - Output format values follow Microsoft Speech output formats (including Ogg/WebM Opus). citeturn1search0�?  - Telegram `sendVoice` accepts OGG/MP3/M4A; use OpenAI/ElevenLabs if you need
    guaranteed Opus voice notes. citeturn1search1�?  - If the configured Edge output format fails, openx retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so openx registers
