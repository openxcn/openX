...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as openxConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as openxConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId, toolContext }) => {
    ...
    const opts = { cfg: cfg as openxConfig, accountId: accountId ?? undefined };
