import type { ChannelAccountSnapshot, ChannelPlugin, openxConfig } from "clawdbot/plugin-sdk";
import {
...
export const bluebubblesPlugin: ChannelPlugin<ResolvedBlueBubblesAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listBlueBubblesAccountIds(cfg as openxConfig),
    resolveAccount: (cfg, accountId) =>
      resolveBlueBubblesAccount({ cfg: cfg as openxConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultBlueBubblesAccountId(cfg as openxConfig),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveBlueBubblesAccount({ cfg: cfg as openxConfig, accountId }).config.allowFrom ??
        []).map(
        (entry) => String(entry),
      ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as openxConfig).channels?.bluebubbles?.accounts?.[resolvedAccountId],
      );
