...
export function listGoogleChatAccountIds(cfg: openxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: openxConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: openxConfig): ResolvedGoogleChatAccount[] {
