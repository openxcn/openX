...
export const googlechatDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveGoogleChatAccount({ cfg: cfg as openxConfig, accountId }).config.dm?.allowFrom ??
        []
...
export const googlechatPlugin: ChannelPlugin<ResolvedGoogleChatAccount> = {
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveGoogleChatAccount({ cfg: cfg as openxConfig });
  ...
  config: {
    listAccountIds: (cfg) => listGoogleChatAccountIds(cfg as openxConfig),
    resolveAccount: (cfg, accountId) =>
      resolveGoogleChatAccount({ cfg: cfg as openxConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultGoogleChatAccountId(cfg as openxConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as openxConfig).channels?.["googlechat"]?.accounts?.[resolvedAccountId],
      );
    ...
    applyAccountConfig: ({ cfg, accountId, input }) => {
      ...
      const next =
        accountId !== DEFAULT_ACCOUNT_ID
          ? migrateBaseNameToDefaultAccount({
              cfg: namedConfig as openxConfig,
              channelKey: "googlechat",
            })
          : namedConfig;
