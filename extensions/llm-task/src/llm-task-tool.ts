...
// NOTE: This extension is intended to be bundled with openx.
// When running from source (tests/dev), openx internals live under src/.
...
export function createLlmTaskTool(api: openxPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-llm-task-"));
