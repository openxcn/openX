...
export function listMattermostAccountIds(cfg: openxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultMattermostAccountId(cfg: openxConfig): string {
...
export function listEnabledMattermostAccounts(cfg: openxConfig): ResolvedMattermostAccount[] {
