...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["openxPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["openx", "pollId"]) ??
    readNestedString(value, ["openx", "poll", "id"]) ??
    readNestedString(value, ["data", "openxPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "openx", "pollId"]);
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "openx poll vote",
          displayText: "Vote recorded",
          value: { openxPollId: pollId },
        },
      },
    ],
  };
