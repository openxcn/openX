...
metadata: {"openx":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == openx `sessions_spawn`
- **File I/O** == openx `read`/`write`
- **Remote fetch** == openx `web_fetch` (or `exec` with curl when POST is required)
