...
function resolveopenxRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.openx_ROOT?.trim() || process.env.CLAWDBOT_ROOT?.trim();
  if (override) {
  ...
  throw new Error(
    "Unable to resolve core root. Set openx_ROOT (or legacy CLAWDBOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveopenxRoot();
