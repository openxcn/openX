...
export const zaloDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZaloAccount({ cfg: cfg as openxConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zaloPlugin: ChannelPlugin<ResolvedZaloAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZaloAccountIds(cfg as openxConfig),
    resolveAccount: (cfg, accountId) => resolveZaloAccount({ cfg: cfg as openxConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZaloAccountId(cfg as openxConfig),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as openxConfig).channels?.zalo?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    ...
    listPeers: async ({ cfg, accountId, query, limit }) => {
      const account = resolveZaloAccount({ cfg: cfg as openxConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZaloAccount({ cfg: cfg as openxConfig });
