...
export const zalouserDock: ChannelDock = {
  ...
  config: {
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveZalouserAccountSync({ cfg: cfg as openxConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
...
export const zalouserPlugin: ChannelPlugin<ResolvedZalouserAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listZalouserAccountIds(cfg as openxConfig),
    resolveAccount: (cfg, accountId) =>
      resolveZalouserAccountSync({ cfg: cfg as openxConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultZalouserAccountId(cfg as openxConfig),
    setAccountEnabled: ({ cfg, accountId, enabled }) =>
      setAccountEnabledInConfigSection({
        cfg: cfg as openxConfig,
        sectionKey: "zalouser",
        accountId,
        enabled,
        allowTopLevel: true,
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as openxConfig).channels?.zalouser?.accounts?.[resolvedAccountId],
      );
  ...
  directory: {
    self: async ({ cfg, accountId, runtime }) => {
      ...
      const account = resolveZalouserAccountSync({ cfg: cfg as openxConfig, accountId });
  ...
  pairing: {
    ...
    notifyApproval: async ({ cfg, id }) => {
      const account = resolveZalouserAccountSync({ cfg: cfg as openxConfig });
  ...
  auth: {
    login: async ({ cfg, accountId, runtime }) => {
      ...
      if (!ok) {
        throw new Error(
          "Missing dependency: `zca` not found in PATH. See docs.openx.bot/channels/zalouser",
        );
      }
      runtime.log(
