...
#
# Suggested cron: */30 * * * * /home/admin/openx/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send openx notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., openx-alerts)
...
send_notification() {
    ...
    # Send via openx if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use openx
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via openx to $NOTIFY_PHONE..."
            openx send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! openx is down. Run: ssh l36 '~/openx/scripts/mobile-reauth.sh'" "urgent"
    exit 1
