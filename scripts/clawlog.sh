...
show_usage() {
    cat << EOF
clawlog - openx Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View openx logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    openx logs flow through the macOS unified log (subsystem: bot.openx).
