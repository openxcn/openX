...
INSTALL_URL="${CLAWDBOT_INSTALL_URL:-https://openx.bot/install.sh}"
...
EXPECTED_VERSION="$(npm view "openx@${INSTALL_TAG}" version)"
if [[ -z "$EXPECTED_VERSION" || "$EXPECTED_VERSION" == "undefined" || "$EXPECTED_VERSION" == "null" ]]; then
  echo "ERROR: unable to resolve openx@${INSTALL_TAG} version" >&2
  exit 2
fi
...
const versions = JSON.parse(execSync("npm view openx versions --json", { encoding: "utf8" }));
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  ...
  npm install -g "openx@${PREVIOUS_VERSION}"
fi
...
INSTALLED_VERSION="$(openx --version 2>/dev/null | head -n 1 | tr -d '\r')"
echo "installed=$INSTALLED_VERSION expected=$EXPECTED_VERSION"
if [[ "$INSTALLED_VERSION" != "$EXPECTED_VERSION" ]]; then
  echo "ERROR: expected openx@$EXPECTED_VERSION, got openx@$INSTALLED_VERSION" >&2
  exit 1
fi
...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if openx --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if openx --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
run_profile() {
  ...
  openx --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 60); do
    if openx --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  openx --profile "$profile" health --timeout 10000 --json >/dev/null
