...
IMAGE_NAME="openx-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/openx-bin:$PATH"
  mkdir -p /tmp/openx-bin

  cat > /tmp/openx-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/openx-bin/systemctl

  cat > /tmp/openx-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/openx-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/openx"
  npm_entry="/tmp/npm-prefix/lib/node_modules/openx/openx.mjs"
  git_entry="/app/openx.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/openx-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/openx-gateway.service"
    if [ ! -f "$unit_path" ]; then
