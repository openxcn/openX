...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/openx-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/openx-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/openx-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.clawdbot/openx.json"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.clawdbot/openx.json" <<'"'"'JSON'"'"'
{
