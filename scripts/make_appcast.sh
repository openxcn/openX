...
ZIP=${1:?"Usage: $0 openx-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/openx/openx/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^openx-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/openx/openx/releases/download/v${VERSION}/"}
