...
# Build and bundle openx into a minimal .app we can open.
# Outputs to dist/openx.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/openx.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="openx"
BUNDLE_ID="${BUNDLE_ID:-bot.openx.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/openx/openx/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/openx/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :openxBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :openxGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/openx"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/openx"
fi
chmod +x "$APP_ROOT/Contents/MacOS/openx"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/openx" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/openx/Resources/openx.icns" "$APP_ROOT/Contents/Resources/openx.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/openx/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying openxKit resources"
openxKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/openxKit_openxKit.bundle"
if [ -d "$openxKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/openxKit_openxKit.bundle"
  cp -R "$openxKIT_BUNDLE" "$APP_ROOT/Contents/Resources/openxKit_openxKit.bundle"
else
  echo "WARN: openxKit resource bundle not found at $openxKIT_BUNDLE (continuing)" >&2
fi
...
echo "�? Stopping any running openx"
killall -q openx 2>/dev/null || true
