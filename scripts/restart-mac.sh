#!/usr/bin/env bash
# Reset openx like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${CLAWDBOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="openx.app/Contents/MacOS/openx"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/openx"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/openx"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/openx"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/bot.openx.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/openx-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${CLAWDBOT_RESTART_LOG:-/tmp/openx-restart.log}"
NO_SIGN=0
SIGN=0
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node openx.mjs daemon install --force --runtime node"
      log "  node openx.mjs daemon restart"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/bot.openx.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing openx instances"
kill_all_openx
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product openx"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/openx.app" ]]; then
    APP_BUNDLE="/Applications/openx.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/openx.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/openx.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/openx.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set CLAWDBOT_APP_BUNDLE to your installed openx.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node openx.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node openx.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".clawdbot", "openx.json"), "utf8");
...
if pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1; then
  log "OK: openx is running."
else
  fail "App exited immediately. Check ${LOG_PATH} or Console.app (User Reports)."
fi

if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/bot.openx.gateway.plist' | head -n 40 || true"
fi
