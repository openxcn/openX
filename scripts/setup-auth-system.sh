#!/bin/bash
# Setup openx Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'openx-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  openx message: Send warning via openx itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/openx-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/openx-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/openx-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now openx-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status openx-auth-monitor.timer"
echo ""
echo "Quick commands:"
