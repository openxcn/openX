...
/**
 * Cleanup suspended openx CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: openxConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-cli-images-"));
  const paths: string[] = [];
