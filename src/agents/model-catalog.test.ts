...
vi.mock("./models-config.js", () => ({
  ensureopenxModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
}));

vi.mock("./agent-paths.js", () => ({
  resolveopenxAgentDir: () => "/tmp/openx",
}));
...
describe("loadModelCatalog", () => {
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as openxConfig });
