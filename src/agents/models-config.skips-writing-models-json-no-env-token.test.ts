...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "openx-models-" });
}

const MODELS_CONFIG: openxConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureopenxModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureopenxModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureopenxModelsJson } = await import("./models-config.js");
      const { resolveopenxAgentDir } = await import("./agent-paths.js");

      await ensureopenxModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveopenxAgentDir(), "models.json");
