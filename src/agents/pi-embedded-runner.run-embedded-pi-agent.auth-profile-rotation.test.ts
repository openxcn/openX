...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): openxConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-workspace-"));
    try {
