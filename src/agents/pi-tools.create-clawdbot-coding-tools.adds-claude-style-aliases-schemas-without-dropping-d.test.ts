...
const defaultTools = createopenxCodingTools();

describe("createopenxCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createopenxCodingTools();
    ...
    try {
      ...
      const contents = "Hello from openx read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "openx-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "openx-workspace"),
    ...
    const tools = createopenxCodingTools({ sandbox });
