...
import { createopenxTools } from "./openx-tools.js";
import { __testing, createopenxCodingTools } from "./pi-tools.js";
...
describe("createopenxCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createopenxCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "openx-outside.txt");
