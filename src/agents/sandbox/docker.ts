...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "openx.sandbox=1");
  args.push("--label", `openx.sessionKey=${params.scopeKey}`);
  args.push("--label", `openx.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `openx.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "openx.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`openx sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`openx sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("openx sandbox recreate --all");
}
