...
type WebSearchConfig = NonNullable<openxConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: openxConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("openx configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.openx.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://openx.bot",
      "X-Title": "openx Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
