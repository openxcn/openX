...
import { scheduleGatewaySigusr1Restart, triggeropenxRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting openx in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggeropenxRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting openx via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
