...
beforeAll(async () => {
  testWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-commands-"));
...
function buildParams(commandBody: string, cfg: openxConfig, ctxOverrides?: Partial<MsgContext>) {
  const ctx = {
    Body: commandBody,
