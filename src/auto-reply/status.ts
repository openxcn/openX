...
type AgentConfig = Partial<NonNullable<NonNullable<openxConfig["agents"]>["defaults"]>>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 openx ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: openxConfig): string {
  const lines = ["ℹ️ Help", ""];
