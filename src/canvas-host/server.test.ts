...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("openxCanvasA2UIAction");
    expect(out).toContain("openxSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("openxSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__openx__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("openx-a2ui-host");
      expect(html).toContain("openxCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__openx__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("openxA2UI");
    } finally {
      await server.close();
      await fs.rm(dir, { recursive: true, force: true });
    }
