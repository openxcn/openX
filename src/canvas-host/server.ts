...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.openxCanvasA2UIAction ||
        window.webkit.messageHandlers.clawdbotCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.openxCanvasA2UIAction &&
        typeof window.openxCanvasA2UIAction.postMessage === "function") ||
  ...
  if (!window.openxSendUserAction && legacySend) {
    window.openxSendUserAction = legacySend;
  }
  if (!window.clawdbotSendUserAction && typeof window.openxSendUserAction === "function") {
    window.clawdbotSendUserAction = window.openxSendUserAction;
  }
  const hasHelper = () =>
    typeof window.openxSendUserAction === "function" ||
  ...
  window.addEventListener("openx:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android openx node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.openxSendUserAction === "function"
        ? window.openxSendUserAction
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>openx Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
