...
import { discoveropenxPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { openxPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  openx?: openxPackageManifest;
  [LEGACY_MANIFEST_KEY]?: openxPackageManifest;
};
...
function toChannelMeta(params: {
  channel: NonNullable<openxPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageopenx?: openxPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageopenx;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.openx ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageopenx: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoveropenxPlugins({ workspaceDir: options.workspaceDir });
