...
function setSlackDmPolicy(cfg: openxConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "openx";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for openx`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: openxConfig, allowFrom: string[]): openxConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
