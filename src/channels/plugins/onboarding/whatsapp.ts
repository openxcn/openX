...
function setWhatsAppDmPolicy(cfg: openxConfig, dmPolicy: DmPolicy): openxConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: openxConfig, allowFrom?: string[]): openxConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: openxConfig, selfChatMode: boolean): openxConfig {
...
async function detectWhatsAppLinked(cfg: openxConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so openx can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for openx" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("openx channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
