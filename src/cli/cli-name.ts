...
export const DEFAULT_CLI_NAME = "openx";
export const LEGACY_CLI_NAME = "openx";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME, LEGACY_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(openx|openx)\b/;

export function resolveCliName(
  argv: string[] = process.argv,
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string {
  const override = env.openx_CLI_NAME?.trim() || env.CLAWDBOT_CLI_NAME?.trim();
