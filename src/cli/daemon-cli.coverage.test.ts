...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    process.env.CLAWDBOT_STATE_DIR = "/tmp/openx-cli-state";
    process.env.CLAWDBOT_CONFIG_PATH = "/tmp/openx-cli-state/openx.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        CLAWDBOT_STATE_DIR: "/tmp/openx-daemon-state",
        CLAWDBOT_CONFIG_PATH: "/tmp/openx-daemon-state/openx.json",
        CLAWDBOT_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/bot.openx.gateway.plist",
    });
