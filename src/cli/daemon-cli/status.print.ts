...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("openx doctor")}" (or "${formatCliCommand("openx doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("openx gateway install --force")}\` from the same --profile / CLAWDBOT_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (service.runtime?.cachedLabel) {
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("openx gateway install")}`),
    );
    spacer();
  }
  ...
  if (legacyServices.length > 0) {
    ...
    defaultRuntime.error(errorText(`Cleanup: ${formatCliCommand("openx doctor")}`));
    spacer();
  }
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("openx status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.openx.bot/troubleshooting`);
}
