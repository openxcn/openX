...
${formatHelpExamples([
  ['openx agent --to +15555550123 --message "status update"', "Start a new session."],
  ['openx agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'openx agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'openx agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['openx agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.openx.bot/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.openx.bot/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['openx agents set-identity --agent main --name "Clawd" --emoji "🦞"', "Set name + emoji."],
  ["openx agents set-identity --agent main --avatar avatars/clawd.png", "Set avatar path."],
  ["openx agents set-identity --workspace ~/clawd --from-identity", "Load from IDENTITY.md."],
  [
    "openx agents set-identity --identity-file ~/clawd/IDENTITY.md --agent main",
