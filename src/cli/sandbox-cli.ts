...
const SANDBOX_EXAMPLES = {
  main: [
    ["openx sandbox list", "List all sandbox containers."],
    ["openx sandbox list --browser", "List only browser containers."],
    ["openx sandbox recreate --all", "Recreate all containers."],
    ["openx sandbox recreate --session main", "Recreate a specific session."],
    ["openx sandbox recreate --agent mybot", "Recreate agent containers."],
    ["openx sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["openx sandbox list", "List all sandbox containers."],
    ["openx sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["openx sandbox recreate --all", "Recreate all containers."],
    ["openx sandbox recreate --session main", "Recreate a specific session."],
    ["openx sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["openx sandbox recreate --browser --all", "Recreate only browser containers."],
    ["openx sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["openx sandbox explain", "Show effective sandbox config."],
    ["openx sandbox explain --session agent:main:main", "Explain a specific session."],
    ["openx sandbox explain --agent work", "Explain an agent sandbox."],
    ["openx sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.openx.bot/cli/sandbox")}\n`,
    )
    .action(() => {
