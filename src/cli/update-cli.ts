...
import { resolveopenxPackageRoot } from "../infra/openx-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "openx doctor": "Running doctor checks",
...
const UPDATE_QUIPS = [
  ...
  "The lobster has openxed. Harder shell, sharper claws.",
  ...
  "openxing complete. Please don't look at my soft shell phase.",
  "Version bump! Same chaos energy, fewer crashes (probably).",
];

const MAX_LOG_CHARS = 8000;
const DEFAULT_PACKAGE_NAME = "openx";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "openx"]);
const CLI_NAME = resolveCliName();
const CLAWDBOT_REPO_URL = "https://github.com/openx/openx.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "openx");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("openx@")) return trimmed.slice("openx@".length);
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWDBOT_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBOT_GIT_DIR to an empty folder or a openx checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("openx update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating openx..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this openx install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("openx doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("openx gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g openx@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g openx@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("openx gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openx doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("openx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("openx gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `openx update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `CLAWDBOT_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBOT_GIT_DIR to an empty folder or a openx checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update openx to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["openx update", "Update a source checkout (git)"],
        ["openx update --channel beta", "Switch to beta channel (git + npm)"],
        ["openx update --channel dev", "Switch to dev channel (git + npm)"],
        ["openx update --tag beta", "One-off update to a dist-tag or version"],
        ["openx update --no-restart", "Update without restarting the service"],
        ["openx update --json", "Output result as JSON"],
        ["openx update --yes", "Non-interactive (accept downgrade prompts)"],
        ["openx update wizard", "Interactive update wizard"],
        ["openx --update", "Shorthand for openx update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run openx update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openx.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["openx update status", "Show channel + version status."],
          ["openx update status --json", "JSON output."],
          ["openx update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.openx.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {
