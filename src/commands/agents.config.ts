...
export function listAgentEntries(cfg: openxConfig): AgentEntry[] {
...
function resolveAgentName(cfg: openxConfig, agentId: string) {
...
function resolveAgentModel(cfg: openxConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: openxConfig): AgentSummary[] {
