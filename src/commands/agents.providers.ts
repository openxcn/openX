...
function resolveDefaultAccountId(cfg: openxConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: openxConfig): boolean {
...
export function summarizeBindings(cfg: openxConfig, bindings: AgentBinding[]): string[] {
