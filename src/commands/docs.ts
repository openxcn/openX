...
const SEARCH_TOOL = "https://docs.openx.bot/mcp.Searchopenx";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.openx.bot");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('openx docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openx.bot/");
      runtime.log(`Search: ${formatCliCommand('openx docs "your query"')}`);
    }
    return;
  }
