...
function stripUnknownConfigKeys(config: openxConfig): {
  config: openxConfig;
  removed: string[];
} {
  const parsed = openxSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as openxConfig;
...
function noteOpencodeProviderOverrides(cfg: openxConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.openx_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "clawdbot.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "openx.json");
  ...
  let cfg: openxConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as openxConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("openx doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("openx doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "openx doctor --fix" to remove these keys.');
    }
  }
