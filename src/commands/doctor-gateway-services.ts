...
export async function maybeMigrateLegacyGatewayService(
  ...
  const migrate = await prompter.confirmSkipInNonInteractive({
    message: "Migrate legacy gateway services to openx now?",
    initialValue: true,
  });
  if (!migrate) return;
  ...
  if (loaded) {
    note(`openx ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install openx gateway service now?",
    initialValue: true,
  });
  if (!install) return;
