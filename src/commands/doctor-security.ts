...
import type { openxConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: openxConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("openx security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("openx configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("openx config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("openx doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("openx config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
