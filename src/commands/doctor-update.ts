...
async function detectopenxGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const git = await detectopenxGitCheckout(params.root);
  if (git === "git") {
    const shouldUpdate = await params.confirm({
      message: "Update openx from git before running doctor?",
      initialValue: true,
    });
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("openx update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
