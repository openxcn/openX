...
beforeEach(() => {
  ...
  resolveopenxPackageRoot.mockReset().mockResolvedValue(null);
  ...
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "openx-doctor-state-"));
...
const resolveopenxPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadopenxPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    ...
    const legacyPath = path.join("/Users/steipete", "openx");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/openx" || value === legacyPath || value === legacyAgentsPath)
        return true;
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("openx"),
      ),
    ).toBe(true);
