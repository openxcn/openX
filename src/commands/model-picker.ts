...
function resolveConfiguredModelRaw(cfg: openxConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: openxConfig): string[] {
...
export function applyPrimaryModel(cfg: openxConfig, model: string): openxConfig {
...
export function applyModelAllowlist(cfg: openxConfig, models: string[]): openxConfig {
  const defaults = cfg.agents?.defaults;
