...
import { readConfigFileSnapshot, type openxConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: openxConfig, model: string): openxConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("openx plugins install")}\`.`,
    );
  }
