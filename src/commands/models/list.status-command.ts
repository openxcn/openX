...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("openx models auth setup-token")}\` or \`${formatCliCommand("openx configure")}\`.`
          : `Run \`${formatCliCommand("openx configure")}\` or set an API key env var.`;
