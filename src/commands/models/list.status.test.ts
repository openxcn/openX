...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveopenxAgentDir: vi.fn().mockReturnValue("/tmp/openx-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveopenxAgentDir: mocks.resolveopenxAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/openx-agent/auth-profiles.json");
