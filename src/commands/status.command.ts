...
export async function statusCommand(
  ...
  runtime.log(theme.heading("openx status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("openx security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("openx security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.openx.bot/faq");
  runtime.log("Troubleshooting: https://docs.openx.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("openx status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("openx logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("openx status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("openx gateway probe")}`);
  }
}
