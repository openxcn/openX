...
vi.mock("../infra/openx-root.js", () => ({
  resolveopenxPackageRoot: vi.fn().mockResolvedValue("/tmp/openx"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/openx/pnpm-lock.yaml",
      markerPath: "/tmp/openx/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.openx.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/bot.openx.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("openx status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("openx status --all") ||
          l.includes("openx --profile isolated status --all") ||
          l.includes("openx status --all") ||
          l.includes("openx --profile isolated status --all"),
      ),
    ).toBe(true);
