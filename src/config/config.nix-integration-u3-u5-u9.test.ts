  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.clawdbot when env not set", async () => {
      await withEnvOverride(
        { openx_STATE_DIR: undefined, CLAWDBOT_STATE_DIR: undefined },
        async () => {
    ...
    it("STATE_DIR respects CLAWDBOT_STATE_DIR override", async () => {
      await withEnvOverride(
        { openx_STATE_DIR: undefined, CLAWDBOT_STATE_DIR: "/custom/state/dir" },
        async () => {
          const { STATE_DIR } = await import("./config.js");
    ...
    it("STATE_DIR prefers openx_STATE_DIR over legacy override", async () => {
      await withEnvOverride(
        { openx_STATE_DIR: "/custom/new", CLAWDBOT_STATE_DIR: "/custom/legacy" },
        async () => {
          const { STATE_DIR } = await import("./config.js");
    ...
    it("CONFIG_PATH defaults to ~/.clawdbot/openx.json when env not set", async () => {
      await withEnvOverride(
        {
          openx_CONFIG_PATH: undefined,
          openx_STATE_DIR: undefined,
          CLAWDBOT_CONFIG_PATH: undefined,
          CLAWDBOT_STATE_DIR: undefined,
        },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toMatch(/\.clawdbot[\\/]openx\.json$/);
        },
      );
    });
    ...
    it("CONFIG_PATH respects CLAWDBOT_CONFIG_PATH override", async () => {
      await withEnvOverride(
        { openx_CONFIG_PATH: undefined, CLAWDBOT_CONFIG_PATH: "/nix/store/abc/openx.json" },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/abc/openx.json"));
        },
      );
    });

    it("CONFIG_PATH prefers openx_CONFIG_PATH over legacy override", async () => {
      await withEnvOverride(
        {
          openx_CONFIG_PATH: "/nix/store/new/openx.json",
          CLAWDBOT_CONFIG_PATH: "/nix/store/legacy/openx.json",
        },
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.resolve("/nix/store/new/openx.json"));
        },
      );
    });
    ...
    it("CONFIG_PATH expands ~ in CLAWDBOT_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        await withEnvOverride(
          { openx_CONFIG_PATH: undefined, CLAWDBOT_CONFIG_PATH: "~/.clawdbot/custom.json" },
          async () => {
            const { CONFIG_PATH } = await import("./config.js");
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", async () => {
      await withEnvOverride(
        ...
        async () => {
          const { CONFIG_PATH } = await import("./config.js");
          expect(CONFIG_PATH).toBe(path.join(path.resolve("/custom/state"), "openx.json"));
        },
      );
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        ...
        await fs.writeFile(
          path.join(pluginDir, "openx.plugin.json"),
          JSON.stringify(
            {
              id: "demo-plugin",
        ...
        await fs.writeFile(
          path.join(configDir, "openx.json"),
          JSON.stringify(
            {
              plugins: {
                load: {
