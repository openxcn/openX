...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "openx-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".openx" | ".clawdbot",
  port: number,
  filename: "openx.json" | "clawdbot.json" = "openx.json",
) {
...
describe("config io compat (new + legacy folders)", () => {
  it("prefers ~/.openx/openx.json when both configs exist", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".openx", 19001);
  ...
  it("falls back to ~/.clawdbot/openx.json when only legacy exists", async () => {
    await withTempHome(async (home) => {
  ...
  it("prefers openx.json over legacy filename in the same dir", async () => {
    await withTempHome(async (home) => {
      const preferred = await writeConfig(home, ".clawdbot", 20003, "openx.json");
  ...
  it("honors explicit legacy config path env override", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".openx", 19002);
