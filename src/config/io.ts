...
import type { openxConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareopenxVersions } from "./version.js";
...
function coerceConfig(value: unknown): openxConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as openxConfig;
}
...
function stampConfigVersion(cfg: openxConfig): openxConfig {
...
function warnIfConfigFromFuture(cfg: openxConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareopenxVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer openx (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: openxConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): openxConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as openxConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as openxConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: openxConfig) {
    clearConfigCache();
...
export function loadConfig(): openxConfig {
...
export async function writeConfigFile(cfg: openxConfig): Promise<void> {
  clearConfigCache();
