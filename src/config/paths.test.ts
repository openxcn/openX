...
describe("state + config path candidates", () => {
  it("prefers openx_STATE_DIR over legacy state dir env", () => {
    const env = {
      openx_STATE_DIR: "/new/state",
  ...
  it("orders default config candidates as new then legacy", () => {
    ...
    expect(candidates[0]).toBe(path.join(home, ".openx", "openx.json"));
    expect(candidates[1]).toBe(path.join(home, ".openx", "clawdbot.json"));
    expect(candidates[2]).toBe(path.join(home, ".clawdbot", "openx.json"));
    expect(candidates[3]).toBe(path.join(home, ".clawdbot", "clawdbot.json"));
  });

  it("prefers ~/.openx when it exists and legacy dir is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openx-state-"));
    try {
      const newDir = path.join(root, ".openx");
  ...
  it("CONFIG_PATH prefers existing legacy filename when present", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openx-config-"));
    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousHomeDrive = process.env.HOMEDRIVE;
    const previousHomePath = process.env.HOMEPATH;
    const previousopenxConfig = process.env.openx_CONFIG_PATH;
    const previousClawdbotConfig = process.env.CLAWDBOT_CONFIG_PATH;
    const previousopenxState = process.env.openx_STATE_DIR;
    ...
    try {
      ...
      delete process.env.openx_STATE_DIR;
      ...
      if (previousopenxConfig === undefined) delete process.env.openx_CONFIG_PATH;
      else process.env.openx_CONFIG_PATH = previousopenxConfig;
      ...
      if (previousopenxState === undefined) delete process.env.openx_STATE_DIR;
      else process.env.openx_STATE_DIR = previousopenxState;
  ...
  it("respects state dir overrides when config is missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "openx-config-override-"));
    try {
      const legacyDir = path.join(root, ".clawdbot");
      await fs.mkdir(legacyDir, { recursive: true });
      const legacyConfig = path.join(legacyDir, "openx.json");
      await fs.writeFile(legacyConfig, "{}", "utf-8");

      const overrideDir = path.join(root, "override");
      const env = { openx_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const resolved = resolveConfigPath(env, overrideDir, () => root);
      expect(resolved).toBe(path.join(overrideDir, "openx.json"));
    } finally {
      await fs.rm(root, { recursive: true, force: true });
    }
  });
