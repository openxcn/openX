...
import type { openxConfig } from "./types.js";
...
const NEW_STATE_DIRNAME = ".openx";
const CONFIG_FILENAME = "openx.json";
 ...
 * Can be overridden via openx_STATE_DIR (preferred) or CLAWDBOT_STATE_DIR (legacy).
 * Default: ~/.clawdbot (legacy default for compatibility)
 * If ~/.openx exists and ~/.clawdbot does not, prefer ~/.openx.
 */
export function resolveStateDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const override = env.openx_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
 ...
 * Can be overridden via openx_CONFIG_PATH (preferred) or CLAWDBOT_CONFIG_PATH (legacy).
 * Default: ~/.clawdbot/openx.json (or $*_STATE_DIR/openx.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, os.homedir),
): string {
  const override = env.openx_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) return resolveUserPath(override);
...
export function resolveConfigPath(
  ...
  const override = env.openx_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) return resolveUserPath(override);
  const stateOverride = env.openx_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const explicit = env.openx_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) return [resolveUserPath(explicit)];

  const candidates: string[] = [];
  const openxStateDir = env.openx_STATE_DIR?.trim();
  if (openxStateDir) {
    candidates.push(path.join(resolveUserPath(openxStateDir), CONFIG_FILENAME));
    candidates.push(path.join(resolveUserPath(openxStateDir), LEGACY_CONFIG_FILENAME));
  }
 ...
 * Default: os.tmpdir()/openx-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `openx-${uid}` : "openx";
