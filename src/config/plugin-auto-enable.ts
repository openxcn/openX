import type { openxConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: openxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: openxConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: openxConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: openxConfig): boolean {
...
function isIMessageConfigured(cfg: openxConfig): boolean {
...
function isWhatsAppConfigured(cfg: openxConfig): boolean {
...
function isGenericChannelConfigured(cfg: openxConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: openxConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: openxConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: openxConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: openxConfig, pluginId: string): boolean {
...
function ensureAllowlisted(cfg: openxConfig, pluginId: string): openxConfig {
...
function enablePluginEntry(cfg: openxConfig, pluginId: string): openxConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
