...
export type ConfigSchema = ReturnType<typeof openxSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when openx writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /openx).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `openx plugins update` to locate install sources).",
  ...
  "session.agentToAgent.maxPingPongTurns":
    "Max reply-back turns between requester and target (0 or more).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/openx",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = openxSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "openxConfig";
