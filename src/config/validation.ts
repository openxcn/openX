...
import type { openxConfig, ConfigValidationIssue } from "./types.js";
import { openxSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: openxConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: openxConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = openxSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as openxConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as openxConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as openxConfig)),
    ),
  };
}
