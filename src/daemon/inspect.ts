...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  return (
    lower.includes("openx_service_marker") &&
    lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
    lower.includes("openx_service_kind") &&
    lower.includes(GATEWAY_SERVICE_KIND.toLowerCase())
  );
}

function isopenxGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  const lowerContents = contents.toLowerCase();
  if (!lowerContents.includes("gateway")) return false;
  return label.startsWith("bot.openx.") || label.startsWith("com.clawdbot.");
}

function isopenxGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  if (!name.startsWith("openx-gateway")) return false;
  return contents.toLowerCase().includes("gateway");
}

function isopenxGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) return false;
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("openx gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (isopenxGatewayLaunchdService(label, contents)) continue;
    results.push({
      platform: "darwin",
      label,
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (!containsMarker(contents)) continue;
    if (isopenxGatewaySystemdService(name, contents)) continue;
    results.push({
      platform: "linux",
      label: entry,
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (!name) continue;
      if (isopenxGatewayTaskName(name)) continue;
