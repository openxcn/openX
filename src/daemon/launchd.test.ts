...
async function withLaunchctlStub(
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-launchctl-test-"));
  try {
...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    await withLaunchctlStub({ listOutput: "123 0 bot.openx.gateway\n" }, async ({ env }) => {
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when CLAWDBOT_PROFILE is default", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.openx.gateway.plist",
    );
  });
  ...
  it("uses profile-specific label when CLAWDBOT_PROFILE is set to a custom value", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.openx.jbphoenix.plist",
    );
  });
  ...
  it("ignores empty CLAWDBOT_LAUNCHD_LABEL and falls back to profile", () => {
    ...
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/bot.openx.myprofile.plist",
    );
  });
