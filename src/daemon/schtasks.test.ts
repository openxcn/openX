...
describe("resolveTaskScriptPath", () => {
  ...
  it("prefers CLAWDBOT_STATE_DIR over profile-derived defaults", () => {
    const env = {
      ...
      CLAWDBOT_STATE_DIR: "C:\\State\\openx",
    };
    expect(resolveTaskScriptPath(env)).toBe(path.join("C:\\State\\openx", "gateway.cmd"));
  });
...
describe("readScheduledTaskCommand", () => {
  it("parses basic command script", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-schtasks-test-"));
    try {
  ...
  it("parses script with working directory", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        scriptPath,
        ["@echo off", "cd /d C:\\Projects\\openx", "node gateway.js"].join("\r\n"),
        "utf8",
      );
      ...
      expect(result).toEqual({
        ...
        workingDirectory: "C:\\Projects\\openx",
      });
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
