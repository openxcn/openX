...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.CLAWDBOT_SERVICE_MARKER).toBe("openx");
    expect(env.CLAWDBOT_SERVICE_KIND).toBe("gateway");
    expect(typeof env.CLAWDBOT_SERVICE_VERSION).toBe("string");
    expect(env.CLAWDBOT_SYSTEMD_UNIT).toBe("openx-gateway.service");
    if (process.platform === "darwin") {
      expect(env.CLAWDBOT_LAUNCHD_LABEL).toBe("bot.openx.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.CLAWDBOT_SYSTEMD_UNIT).toBe("openx-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.CLAWDBOT_LAUNCHD_LABEL).toBe("bot.openx.work");
    }
  });
});
