...
import { resolveopenxAgentDir } from "../agents/agent-paths.js";
...
import { ensureopenxModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { openxConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<openxConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveopenxAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-live-state-"));
  ...
  const agentDir = resolveopenxAgentDir();
  const sanitizedCfg: openxConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "openx-live-"));
  const tempConfigPath = path.join(tempDir, "openx.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CLAWDBOT_CONFIG_PATH = tempConfigPath;

  await ensureopenxModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "openx live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureopenxModelsJson(cfg);
