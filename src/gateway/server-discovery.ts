...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "openx";
  if (/openx/i.test(trimmed)) return trimmed;
  return `${trimmed} (openx)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const siblingCli = path.join(execDir, "openx");
  ...
  const binCli = path.join(cwd, "bin", "openx.js");
