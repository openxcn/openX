...
import { createopenxTools } from "../agents/openx-tools.js";
import {
  filterToolsByPolicy,
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-openx-message-channel") ?? "");
  const accountId = getHeader(req, "x-openx-account-id")?.trim() || undefined;
  ...
  const allTools = createopenxTools({
