...
`;
    ...
    expect(parsed.openx.emoji).toBe("💾");
    expect(parsed.openx.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.openx.emoji).toBe("📝");
    expect(parsed.openx.events).toEqual(["command"]);
    expect(parsed.openx.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.openx.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"openx": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"openx": {"events": ["test"]}}');
  });
...
describe("resolveopenxMetadata", () => {
  it("extracts openx metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveopenxMetadata(frontmatter);
  ...
  it("returns undefined when openx key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        openx: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with openx" },
            { id: "npm", kind: "npm", package: "@openx/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@openx/hook");
  });
...
`;
    ...
    const openx = resolveopenxMetadata(frontmatter);
    expect(openx).toBeDefined();
    expect(openx?.emoji).toBe("💾");
    expect(openx?.events).toEqual(["command:new"]);
    expect(openx?.requires?.config).toEqual(["workspace.dir"]);
    expect(openx?.install?.[0].kind).toBe("bundled");
  });
