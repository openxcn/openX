...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openx-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@openx/zip-hooks",
        version: "0.0.1",
        openx: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
      ...
      [
        ...
        'metadata: {"openx":{"events":["command:new"]}}',
        "---",
        "",
        "# Zip Hook",
      ].join("\n"),
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openx/tar-hooks",
        version: "0.0.1",
        openx: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
