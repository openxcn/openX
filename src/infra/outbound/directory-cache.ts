...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: openxConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: openxConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: openxConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: openxConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: openxConfig): void {
