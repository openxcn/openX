...
function candidateBinDirs(opts: EnsureopenxPathOpts): string[] {
  ...
  // Bundled macOS app: `openx` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingopenx = path.join(execDir, "openx");
    if (isExecutable(siblingopenx)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/openx` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "openx"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `openx` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureopenxCliOnPath(opts: EnsureopenxPathOpts = {}) {
