...
export const WIDE_AREA_DISCOVERY_DOMAIN = "openx.internal.";
export const WIDE_AREA_ZONE_FILENAME = "openx.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*openx-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "openx";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "openx");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "openx-gw");
  ...
  records.push(`_openx-gw._tcp IN PTR ${instanceLabel}._openx-gw._tcp`);
  records.push(`${instanceLabel}._openx-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._openx-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; openx-content-hash: ${contentHash}\n${contentBody}`;
}
