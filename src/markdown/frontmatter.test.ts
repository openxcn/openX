...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { openx?: { emoji?: string } };
    expect(parsed.openx?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { openx?: { events?: string[] } };
