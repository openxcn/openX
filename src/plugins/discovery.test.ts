...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openx-plugins-${randomUUID()}`);
...
describe("discoveropenxPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoveropenxPlugins } = await import("./discovery.js");
      return discoveropenxPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        openx: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@openx/voice-call",
        openx: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openx/demo-plugin-dir",
        openx: { extensions: ["./index.js"] },
      }),
      "utf-8",
    );
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoveropenxPlugins } = await import("./discovery.js");
      return discoveropenxPlugins({ extraPaths: [packDir] });
    });
