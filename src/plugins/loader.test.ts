...
import { loadopenxPlugins } from "./loader.js";
...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `openx-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "openx.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
describe("loadopenxPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadopenxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadopenxPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@openx/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        openx: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
