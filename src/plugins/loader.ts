...
import { discoveropenxPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: openxPluginDefinition;
  register?: openxPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as openxPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as openxPluginDefinition;
...
export function loadopenxPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoveropenxPlugins({
    workspaceDir: options.workspaceDir,
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            ...
            "openx/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: openxPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as openxPluginModule;
    } catch (err) {
