...
export function listSignalAccountIds(cfg: openxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: openxConfig): string {
...
function mergeSignalAccountConfig(cfg: openxConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: openxConfig): ResolvedSignalAccount[] {
