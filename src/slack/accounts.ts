...
export function listSlackAccountIds(cfg: openxConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: openxConfig): string {
...
function mergeSlackAccountConfig(cfg: openxConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: openxConfig): ResolvedSlackAccount[] {
