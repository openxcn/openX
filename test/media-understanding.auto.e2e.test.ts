...
import type { openxConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("openx-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("openx-bin-sherpa-");
      const modelDir = await makeTempDir("openx-sherpa-model-");
      ...
      const cfg: openxConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openx-bin-whispercpp-");
      const modelDir = await makeTempDir("openx-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("openx-bin-gemini-");
      ...
      const cfg: openxConfig = { tools: { media: { image: {} } } };
