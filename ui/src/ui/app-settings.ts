...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as openxApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as openxApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as openxApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as openxApp);
    await loadDevices(host as unknown as openxApp);
    await loadConfig(host as unknown as openxApp);
    await loadExecApprovals(host as unknown as openxApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as openxApp);
    await loadConfig(host as unknown as openxApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as openxApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as openxApp, { reset: true });
    scheduleLogsScroll(
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as openxApp, false),
    loadPresence(host as unknown as openxApp),
    loadSessions(host as unknown as openxApp),
    loadCronStatus(host as unknown as openxApp),
    loadDebug(host as unknown as openxApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as openxApp, true),
    loadConfigSchema(host as unknown as openxApp),
    loadConfig(host as unknown as openxApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as openxApp, false),
    loadCronStatus(host as unknown as openxApp),
    loadCronJobs(host as unknown as openxApp),
  ]);
}
