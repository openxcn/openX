...
import { openxApp } from "./app";
import "../styles.css";

const originalConnect = openxApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("openx-app") as openxApp;
...
afterEach(() => {
  openxApp.prototype.connect = originalConnect;
  window.__CLAWDBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/openx/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/openx");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/openx/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CLAWDBOT_CONTROL_UI_BASE_PATH__ = "/openx";
    const app = mountApp("/openx/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/openx");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/openx/sessions");
  });
